# MazelClaw ACP Bridge

This document describes how the MazelClaw ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`mazelclaw acp` exposes an ACP agent over stdio and forwards prompts to a running
MazelClaw Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
- ACP client spawns `mazelclaw acp` and speaks ACP messages over stdio.
...
`mazelclaw acp` resolves the Gateway URL and auth from CLI flags or config:
