---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for MazelClaw deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, MazelClaw cron scheduling for periodic checks, or version status checks on a machine running MazelClaw (laptop, workstation, Pi, VPS).
---

# MazelClaw Host Hardening

## Overview

Assess and harden the host running MazelClaw, then align it to a user-defined risk tolerance without breaking access. Use MazelClaw security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim MazelClaw changes the host firewall, SSH, or OS updates; it does not.
...
5. MazelClaw gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if MazelClaw can access sensitive data.
...
### 2) Run MazelClaw security audits (read-only)

As part of the default read-only checks, run `mazelclaw security audit --deep`. Only offer alternatives if the user requests them:

1. `mazelclaw security audit` (faster, non-probing)
2. `mazelclaw security audit --json` (structured output)

Offer to apply MazelClaw safe defaults (numbered):

1. `mazelclaw security audit --fix`

Be explicit that `--fix` only tightens MazelClaw defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check MazelClaw version/update status (read-only)

As part of the default read-only checks, run `mazelclaw update status`.
...
- Credential hygiene notes (location of MazelClaw creds, prefer disk encryption)
...
- MazelClaw security audit (re-run)
...
After MazelClaw install or first hardening pass, run at least one baseline audit and version check:

- `mazelclaw security audit`
- `mazelclaw security audit --deep`
- `mazelclaw update status`

Ongoing monitoring is recommended. Use the MazelClaw cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `mazelclaw cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `mazelclaw update status`
...
Before creating, `mazelclaw cron list` and match on exact `name`. If found, `mazelclaw cron edit <id> ...`.
If not found, `mazelclaw cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `mazelclaw update status` (preferred for source checkouts and channels)
2. `npm view mazelclaw version` (published npm version)

## MazelClaw command accuracy

Use only supported commands and flags:

- `mazelclaw security audit [--deep] [--fix] [--json]`
- `mazelclaw status` / `mazelclaw status --deep`
- `mazelclaw health --json`
- `mazelclaw update status`
- `mazelclaw cron add|list|runs|run`

Do not invent CLI flags or imply MazelClaw enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by MazelClaw compaction:
