...
import type { ConfigFileSnapshot } from "./types.mazelclaw.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.mazelclaw/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/mazelclaw/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/mazelclaw/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/mazelclaw/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/mazelclaw/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = MazelClawSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "MazelClawConfig";
    const hints = mapSensitivePaths(MazelClawSchema, "", {});
