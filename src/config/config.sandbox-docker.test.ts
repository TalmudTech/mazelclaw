...
describe("sandbox browser binds config", () => {
  ...
  it("defaults browser network to dedicated sandbox network", () => {
    ...
    expect(resolved.network).toBe("mazelclaw-sandbox-browser");
  });

  it("prefers agent browser network over global browser network", () => {
    const resolved = resolveSandboxBrowserConfig({
      scope: "agent",
      globalBrowser: { network: "mazelclaw-sandbox-browser-global" },
      agentBrowser: { network: "mazelclaw-sandbox-browser-agent" },
    });
    expect(resolved.network).toBe("mazelclaw-sandbox-browser-agent");
  });
