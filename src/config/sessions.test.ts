...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mazelclaw-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ MAZELCLAW_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-mazelclaw",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-mazelclaw#general");
  });
  ...
  it("derives session transcripts dir from MAZELCLAW_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { MAZELCLAW_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent absolute sessionFile paths", () => {
    const stateDir = path.resolve("/home/user/.mazelclaw");
  ...
  it("resolves cross-agent paths when MAZELCLAW_STATE_DIR differs from stored paths", () => {
  ...
  it("rejects absolute sessionFile paths outside agent sessions directories", () => {
    withStateDir(path.resolve("/home/user/.mazelclaw"), () => {
      expect(() =>
        resolveSessionFilePath(
          "sess-1",
