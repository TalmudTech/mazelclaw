...
import type { MazelClawConfig, ConfigValidationIssue } from "./types.js";
import { MazelClawSchema } from "./zod-schema.js";
...
function validateIdentityAvatar(config: MazelClawConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: MazelClawConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = MazelClawSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as MazelClawConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as MazelClawConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as MazelClawConfig,
  };
}
