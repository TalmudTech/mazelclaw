...
describe("skills-cli (e2e)", () => {
  ...
  beforeAll(() => {
    ...
    tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "mazelclaw-skills-test-"));
    tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "mazelclaw-bundled-skills-test-"));
    process.env.MAZELCLAW_BUNDLED_SKILLS_DIR = tempBundledDir;
  });
