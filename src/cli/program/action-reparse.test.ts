...
describe("reparseProgramFromActionArgs", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    buildParseArgvMock.mockReturnValue(["node", "mazelclaw", "status"]);
  ...
  it("uses action command name + args as fallback argv", async () => {
    ...
    const actionCommand = {
      name: () => "status",
      parent: {
        rawArgs: ["node", "mazelclaw", "status", "--json"],
      },
    ...
    expect(parseAsync).toHaveBeenCalledWith(["node", "mazelclaw", "status"]);
  });
