...
beforeEach(() => {
  ...
  originalHideBanner = process.env.MAZELCLAW_HIDE_BANNER;
  delete process.env.NODE_NO_WARNINGS;
  delete process.env.MAZELCLAW_HIDE_BANNER;
});
...
afterEach(() => {
  ...
  if (originalHideBanner === undefined) {
    ...
    process.env.MAZELCLAW_HIDE_BANNER = originalHideBanner;
  }
});

describe("registerPreActionHooks", () => {
  function buildProgram() {
    const program = new Command().name("mazelclaw");
  ...
  it("emits banner, resolves config, and enables verbose from --debug", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "mazelclaw", "status", "--debug"],
    });
    ...
    expect(process.title).toBe("mazelclaw-status");
  });

  it("loads plugin registry for plugin-required commands", async () => {
    await runCommand({
      parseArgv: ["message", "send"],
      processArgv: ["node", "mazelclaw", "message", "send"],
    });
  ...
  it("skips config guard for doctor and completion commands", async () => {
    await runCommand({
      parseArgv: ["doctor"],
      processArgv: ["node", "mazelclaw", "doctor"],
    });
    await runCommand({
      parseArgv: ["completion"],
      processArgv: ["node", "mazelclaw", "completion"],
    });
  ...
  it("skips preaction work when argv indicates help/version", async () => {
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "mazelclaw", "--version"],
    });
  ...
  it("hides banner when MAZELCLAW_HIDE_BANNER is truthy", async () => {
    process.env.MAZELCLAW_HIDE_BANNER = "1";
    await runCommand({
      parseArgv: ["status"],
      processArgv: ["node", "mazelclaw", "status"],
    });
