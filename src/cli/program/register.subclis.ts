...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.MAZELCLAW_DISABLE_LAZY_SUBCOMMANDS)) {
    return false;
  }
...
const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.MAZELCLAW_DISABLE_LAZY_SUBCOMMANDS);
};

const loadConfig = async (): Promise<MazelClawConfig> => {
...
const entries: SubCliEntry[] = [
  ...
  {
    name: "docs",
    description: "Search the live MazelClaw docs",
    hasSubcommands: false,
    register: async (program) => {
