...
${formatHelpExamples([
  ['mazelclaw agent --to +15555550123 --message "status update"', "Start a new session."],
  ['mazelclaw agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'mazelclaw agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'mazelclaw agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['mazelclaw agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'mazelclaw agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.mazelclaw.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.mazelclaw.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['mazelclaw agents set-identity --agent main --name "MazelClaw" --emoji "🦞"', "Set name + emoji."],
  ["mazelclaw agents set-identity --agent main --avatar avatars/mazelclaw.png", "Set avatar path."],
  [
    "mazelclaw agents set-identity --workspace ~/.mazelclaw/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "mazelclaw agents set-identity --identity-file ~/.mazelclaw/workspace/IDENTITY.md --agent main",
