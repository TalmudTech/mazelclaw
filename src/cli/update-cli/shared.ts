...
import { resolveMazelClawPackageRoot } from "../../infra/mazelclaw-root.js";
...
const MAZELCLAW_REPO_URL = "https://github.com/mazelclaw/mazelclaw.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "mazelclaw";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("mazelclaw@")) {
    return trimmed.slice("mazelclaw@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.MAZELCLAW_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveMazelClawPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", MAZELCLAW_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `MAZELCLAW_GIT_DIR points at a non-git directory: ${params.dir}. Set MAZELCLAW_GIT_DIR to an empty folder or an mazelclaw checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`MAZELCLAW_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "mazelclaw.mjs");
