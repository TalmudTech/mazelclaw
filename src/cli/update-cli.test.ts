...
import type { MazelClawConfig, ConfigFileSnapshot } from "../config/types.mazelclaw.js";
...
vi.mock("../infra/mazelclaw-root.js", () => ({
  resolveMazelClawPackageRoot: vi.fn(),
}));
...
const { resolveMazelClawPackageRoot } = await import("../infra/mazelclaw-root.js");
...
describe("update-cli", () => {
  const fixtureRoot = "/tmp/mazelclaw-update-tests";
  let fixtureCount = 0;
  ...
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/mazelclaw-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveMazelClawPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = createCaseDir("mazelclaw-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveMazelClawPackageRoot).mockReset();
    ...
    vi.mocked(resolveMazelClawPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("mazelclaw");
    ...
    prepareRestartScript.mockResolvedValue("/tmp/mazelclaw-restart-test.sh");
    runRestartScript.mockResolvedValue(undefined);
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners: [{ pid: 4242, command: "mazelclaw-gateway" }],
      hints: [],
    });
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("MazelClaw update status");
  });
    ...
    },
    {
      ...
      prepare: async () => {
        vi.mocked(readConfigFileSnapshot).mockResolvedValue({
          ...baseSnapshot,
          config: { update: { channel: "beta" } } as MazelClawConfig,
        });
      },
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveMazelClawPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue(
      makeOkUpdateResult({
        mode: "npm",
      }),
  ...
  it("updateCommand refreshes service env from updated install root when available", async () => {
    const root = createCaseDir("mazelclaw-updated-root");
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    try {
      await withEnvAsync({ MAZELCLAW_UPDATE_IN_PROGRESS: undefined }, async () => {
        vi.mocked(runGatewayUpdate).mockResolvedValue(makeOkUpdateResult());
        vi.mocked(runDaemonRestart).mockResolvedValue(true);
        ...
        expect(process.env.MAZELCLAW_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = createCaseDir("mazelclaw-update-wizard");
    await withEnvAsync({ MAZELCLAW_GIT_DIR: tempDir }, async () => {
      setTty(true);
