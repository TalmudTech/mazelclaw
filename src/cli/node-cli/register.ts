    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "mazelclaw node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["mazelclaw node status", "Check node host service status."],
          ["mazelclaw node install", "Install the node host service."],
          ["mazelclaw node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.mazelclaw.ai/cli/node")}\n`,
    );
