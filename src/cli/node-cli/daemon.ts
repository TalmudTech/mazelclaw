...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("mazelclaw node install"), formatCliCommand("mazelclaw node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("mazelclaw node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    MAZELCLAW_LOG_PREFIX: baseEnv.MAZELCLAW_LOG_PREFIX ?? "node",
