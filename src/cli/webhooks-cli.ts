...
export function registerWebhooksCli(program: Command) {
  const webhooks = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/webhooks", "docs.mazelclaw.ai/cli/webhooks")}\n`,
    );
  ...
  gmail
    .command("setup")
    .description("Configure Gmail watch + Pub/Sub + MazelClaw hooks")
    ...
    .option("--hook-url <url>", "MazelClaw hook URL")
    .option("--hook-token <token>", "MazelClaw hook token")
