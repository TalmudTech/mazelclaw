    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["mazelclaw nodes status", "List known nodes with live status."],
          ["mazelclaw nodes pairing pending", "Show pending node pairing requests."],
          ['mazelclaw nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["mazelclaw nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.mazelclaw.ai/cli/nodes")}\n`,
    );
