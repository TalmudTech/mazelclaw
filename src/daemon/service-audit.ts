...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.MAZELCLAW_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service MAZELCLAW_GATEWAY_TOKEN does not match gateway.auth.token in mazelclaw.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `mazelclaw gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
