...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.MAZELCLAW_GATEWAY_PORT).toBe("18789");
    expect(env.MAZELCLAW_GATEWAY_TOKEN).toBe("secret");
    expect(env.MAZELCLAW_SERVICE_MARKER).toBe("mazelclaw");
    expect(env.MAZELCLAW_SERVICE_KIND).toBe("gateway");
    expect(typeof env.MAZELCLAW_SERVICE_VERSION).toBe("string");
    expect(env.MAZELCLAW_SYSTEMD_UNIT).toBe("mazelclaw-gateway.service");
    if (process.platform === "darwin") {
      expect(env.MAZELCLAW_LAUNCHD_LABEL).toBe("ai.mazelclaw.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", MAZELCLAW_PROFILE: "work" },
      port: 18789,
    });
    expect(env.MAZELCLAW_SYSTEMD_UNIT).toBe("mazelclaw-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.MAZELCLAW_LAUNCHD_LABEL).toBe("ai.mazelclaw.work");
    }
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".mazelclaw"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", MAZELCLAW_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".mazelclaw-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", MAZELCLAW_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".mazelclaw"));
  });

  it("uses MAZELCLAW_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", MAZELCLAW_STATE_DIR: "/var/lib/mazelclaw" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/mazelclaw"));
  });

  it("expands ~ in MAZELCLAW_STATE_DIR", () => {
    const env = { HOME: "/Users/test", MAZELCLAW_STATE_DIR: "~/mazelclaw-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/mazelclaw-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { MAZELCLAW_STATE_DIR: "C:\\State\\mazelclaw" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\mazelclaw");
  });
});
