...
function listAgents(cfg: MazelClawConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: MazelClawConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: MazelClawConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<MazelClawConfig, EvaluatedBindingsCache>();
