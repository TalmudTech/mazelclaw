...
async function main() {
  ...
  const { ensureMazelClawCliOnPath } = await import("../infra/path-env.js");
  ...
  process.on("uncaughtException", (error) => {
    console.error("[mazelclaw] Uncaught exception:", formatUncaughtError(error));
    process.exit(1);
  });
