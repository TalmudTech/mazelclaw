...
describe("parseRelaySmokeTest", () => {
  ...
  it("parses env var smoke mode only when no args", () => {
    expect(parseRelaySmokeTest([], { MAZELCLAW_SMOKE_QR: "1" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { MAZELCLAW_SMOKE_QR: "1" })).toBe(null);
  });

  it("supports MAZELCLAW_SMOKE=qr only when no args", () => {
    expect(parseRelaySmokeTest([], { MAZELCLAW_SMOKE: "qr" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { MAZELCLAW_SMOKE: "qr" })).toBe(null);
  });
  ...
  it("prefers explicit --smoke over env vars", () => {
    expect(parseRelaySmokeTest(["--smoke", "qr"], { MAZELCLAW_SMOKE: "nope" })).toBe("qr");
  });
});
