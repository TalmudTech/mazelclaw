...
`;
    ...
    expect(parsed.mazelclaw.emoji).toBe("💾");
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.mazelclaw.emoji).toBe("📝");
    expect(parsed.mazelclaw.events).toEqual(["command"]);
    expect(parsed.mazelclaw.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.mazelclaw.install[0].kind).toBe("bundled");
  });
...
describe("resolveMazelClawMetadata", () => {
  it("extracts mazelclaw metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveMazelClawMetadata(frontmatter);
  ...
  it("returns undefined when mazelclaw key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        mazelclaw: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with MazelClaw" },
            { id: "npm", kind: "npm", package: "@mazelclaw/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@mazelclaw/hook");
  });
    ...
    "mazelclaw":
      {
        ...
        "events": ["command:new", "command:reset"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with MazelClaw" }],
      },
  }
---
...
`;
    ...
    const mazelclaw = resolveMazelClawMetadata(frontmatter);
    expect(mazelclaw).toBeDefined();
    expect(mazelclaw?.emoji).toBe("💾");
    expect(mazelclaw?.events).toEqual(["command:new", "command:reset"]);
    expect(mazelclaw?.requires?.config).toEqual(["workspace.dir"]);
    expect(mazelclaw?.install?.[0].kind).toBe("bundled");
  });
...
`;
    const frontmatter = parseFrontmatter(content);
    const mazelclaw = resolveMazelClawMetadata(frontmatter);
    expect(mazelclaw?.emoji).toBe("disk");
    expect(mazelclaw?.events).toEqual(["command:new"]);
  });
});
