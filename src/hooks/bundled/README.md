...
This directory contains hooks that ship with MazelClaw. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.mazelclaw/mazelclaw.json`:
...
2. Restart gateway: `pkill -9 -f 'mazelclaw.*gateway' && pnpm mazelclaw gateway`
3. Enable the hook: `mazelclaw hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.mazelclaw.ai/automation/hooks
