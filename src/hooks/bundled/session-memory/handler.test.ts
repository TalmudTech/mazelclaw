...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => MazelClawConfig;
  action?: "new" | "reset";
}): Promise<{ tempDir: string; files: string[]; memoryContent: string }> {
  const tempDir = await makeTempWorkspace("mazelclaw-session-memory-");
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): MazelClawConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
