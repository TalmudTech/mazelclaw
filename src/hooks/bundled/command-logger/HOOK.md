...
homepage: https://docs.mazelclaw.ai/automation/hooks#command-logger
metadata:
  {
    "mazelclaw":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.mazelclaw/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.mazelclaw/logs/commands.log ~/.mazelclaw/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/mazelclaw`:
   ```
   /home/username/.mazelclaw/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
