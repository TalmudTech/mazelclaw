...
describe("Slack native command argument menus", () => {
  ...
  beforeAll(async () => {
    ...
    argMenuHandler = requireHandler(harness.actions, "mazelclaw_cmdarg", "arg-menu action");
    argMenuOptionsHandler = requireHandler(harness.options, "mazelclaw_cmdarg", "arg-menu options");
  });
  ...
  it("shows a static_select menu when choices exceed button row size", async () => {
    ...
    expect(element?.action_id).toBe("mazelclaw_cmdarg");
  ...
  it("serves filtered options for external_select menus", async () => {
    ...
    expect(blockId).toContain("mazelclaw_cmdarg_ext:");
