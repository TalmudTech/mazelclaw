...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "mazelclaw-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    MAZELCLAW_GATEWAY_TOKEN: "test-token",
    MAZELCLAW_CONFIG_DIR: join(sandbox.rootDir, "config"),
    MAZELCLAW_WORKSPACE_DIR: join(sandbox.rootDir, "mazelclaw"),
  };
...
describe("docker-setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = spawnSync("bash", [sandbox.scriptPath], {
      ...
      env: createEnv(sandbox, {
        MAZELCLAW_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
        MAZELCLAW_EXTRA_MOUNTS: undefined,
        MAZELCLAW_HOME_VOLUME: "mazelclaw-home",
      }),
      stdio: ["ignore", "ignore", "pipe"],
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(sandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("MAZELCLAW_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("MAZELCLAW_EXTRA_MOUNTS=");
    expect(envFile).toContain("MAZELCLAW_HOME_VOLUME=mazelclaw-home");
    const extraCompose = await readFile(join(sandbox.rootDir, "docker-compose.extra.yml"), "utf8");
    expect(extraCompose).toContain("mazelclaw-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("mazelclaw-home:");
    const log = await readFile(sandbox.logPath, "utf8");
    expect(log).toContain("--build-arg MAZELCLAW_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });

  it("rejects injected multiline MAZELCLAW_EXTRA_MOUNTS values", async () => {
    if (!sandbox) {
      throw new Error("sandbox missing");
    }
    ...
    const result = spawnSync("bash", [sandbox.scriptPath], {
      cwd: sandbox.rootDir,
      env: createEnv(sandbox, {
        MAZELCLAW_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
      }),
      encoding: "utf8",
      stdio: ["ignore", "ignore", "pipe"],
    });
    ...
    expect(result.stderr).toContain("MAZELCLAW_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid MAZELCLAW_EXTRA_MOUNTS mount format", async () => {
    if (!sandbox) {
      throw new Error("sandbox missing");
    }
    ...
    const result = spawnSync("bash", [sandbox.scriptPath], {
      cwd: sandbox.rootDir,
      env: createEnv(sandbox, {
        MAZELCLAW_EXTRA_MOUNTS: "bad mount spec",
      }),
      encoding: "utf8",
      stdio: ["ignore", "ignore", "pipe"],
    });
  ...
  it("rejects invalid MAZELCLAW_HOME_VOLUME names", async () => {
    if (!sandbox) {
      throw new Error("sandbox missing");
    }
    ...
    const result = spawnSync("bash", [sandbox.scriptPath], {
      cwd: sandbox.rootDir,
      env: createEnv(sandbox, {
        MAZELCLAW_HOME_VOLUME: "bad name",
      }),
      encoding: "utf8",
      stdio: ["ignore", "ignore", "pipe"],
    });
    ...
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("MAZELCLAW_HOME_VOLUME must match");
  });
