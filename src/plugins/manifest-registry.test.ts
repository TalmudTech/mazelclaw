...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `mazelclaw-manifest-registry-${randomUUID()}`);
...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "mazelclaw.plugin.json"), JSON.stringify(manifest), "utf-8");
}
