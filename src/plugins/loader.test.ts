...
import { loadMazelClawPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `mazelclaw-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.MAZELCLAW_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "mazelclaw.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.MAZELCLAW_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadMazelClawPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    delete process.env.MAZELCLAW_BUNDLED_PLUGINS_DIR;
  } else {
    process.env.MAZELCLAW_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadMazelClawPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const registry = loadMazelClawPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
    ...
    const enabledRegistry = loadMazelClawPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
  ...
  it("loads plugins from config paths", () => {
    process.env.MAZELCLAW_BUNDLED_PLUGINS_DIR = "/nonexistent/bundled/plugins";
  ...
  it("warns when loaded non-bundled plugin has no install/load-path provenance", () => {
    ...
    withEnv({ MAZELCLAW_STATE_DIR: stateDir, CLAWDBOT_STATE_DIR: undefined }, () => {
