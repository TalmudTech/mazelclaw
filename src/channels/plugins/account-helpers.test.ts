...
function cfg(accounts?: Record<string, unknown> | null): MazelClawConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as MazelClawConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as MazelClawConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as MazelClawConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as MazelClawConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as MazelClawConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as MazelClawConfig)).toBe("default");
    });
  });
});
