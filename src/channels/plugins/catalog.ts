...
import { discoverMazelClawPlugins } from "../../plugins/discovery.js";
import type { MazelClawPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, MazelClawPackageManifest>>;
...
const ENV_CATALOG_PATHS = ["MAZELCLAW_PLUGIN_CATALOG_PATHS", "MAZELCLAW_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<MazelClawPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: MazelClawPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageManifest?: MazelClawPackageManifest;
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverMazelClawPlugins({ workspaceDir: options.workspaceDir });
