...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "MAZELCLAW_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("MAZELCLAW_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv MAZELCLAW_GATEWAY_TOKEN");
    expect(message).not.toContain("MAZELCLAW_GATEWAY_PASSWORD");
  });
