...
beforeAll(() => {
  envSnapshot = captureEnv(["MAZELCLAW_PROFILE"]);
  process.env.MAZELCLAW_PROFILE = "isolated";
});
...
vi.mock("../infra/mazelclaw-root.js", () => ({
  resolveMazelClawPackageRoot: vi.fn().mockResolvedValue("/tmp/mazelclaw"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/mazelclaw/pnpm-lock.yaml",
      markerPath: "/tmp/mazelclaw/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("MazelClaw status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("mazelclaw status --all") ||
          l.includes("mazelclaw --profile isolated status --all") ||
          l.includes("mazelclaw status --all") ||
          l.includes("mazelclaw --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.MAZELCLAW_GATEWAY_TOKEN = "abcd1234";
    try {
