...
const SEARCH_TOOL = "https://docs.mazelclaw.ai/mcp.SearchMazelClaw";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.mazelclaw.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('mazelclaw docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.mazelclaw.ai/");
      runtime.log(`Search: ${formatCliCommand('mazelclaw docs "your query"')}`);
    }
    return;
  }
