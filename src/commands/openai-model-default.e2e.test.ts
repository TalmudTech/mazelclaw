...
function expectPrimaryModelChanged(
  applied: { changed: boolean; next: MazelClawConfig },
  primary: string,
) {
...
type SharedDefaultModelCase = {
  apply: (cfg: MazelClawConfig) => { changed: boolean; next: MazelClawConfig };
  defaultModel: string;
  overrideConfig: MazelClawConfig;
  alreadyDefaultConfig: MazelClawConfig;
};
...
describe("applyDefaultModelChoice", () => {
  it("ensures allowlist entry exists when returning an agent override", async () => {
    ...
    const applied = await applyDefaultModelChoice({
      ...
      applyProviderConfig: (config: MazelClawConfig) => config,
      applyDefaultConfig: (config: MazelClawConfig) => config,
      noteAgentModel,
...
describe("shared default model behavior", () => {
  it("sets defaults when model is unset", () => {
    for (const testCase of SHARED_DEFAULT_MODEL_CASES) {
      const cfg: MazelClawConfig = { agents: { defaults: {} } };
