...
const ensureMazelClawModelsJson = vi.fn().mockResolvedValue(undefined);
const ensurePiAuthJsonFromAuthProfiles = vi
  .fn()
  .mockResolvedValue({ wrote: false, authPath: "/tmp/mazelclaw-agent/auth.json" });
const resolveMazelClawAgentDir = vi.fn().mockReturnValue("/tmp/mazelclaw-agent");
...
vi.mock("../config/config.js", () => ({
  ...
  STATE_DIR: "/tmp/mazelclaw-state",
  loadConfig,
}));

vi.mock("../agents/models-config.js", () => ({
  ensureMazelClawModelsJson,
}));
...
describe("models list/status", () => {
  ...
  it("models list syncs auth-profiles into auth.json before availability checks", async () => {
    ...
    expect(ensurePiAuthJsonFromAuthProfiles).toHaveBeenCalledWith("/tmp/mazelclaw-agent");
  });
