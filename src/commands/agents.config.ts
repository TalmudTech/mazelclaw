...
function resolveAgentName(cfg: MazelClawConfig, agentId: string) {
...
function resolveAgentModel(cfg: MazelClawConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: MazelClawConfig): AgentSummary[] {
