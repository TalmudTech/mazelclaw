...
export function requireMazelClawAgentDir(): string {
  const agentDir = process.env.MAZELCLAW_AGENT_DIR;
  if (!agentDir) {
    throw new Error("MAZELCLAW_AGENT_DIR not set");
  }
  return agentDir;
}
