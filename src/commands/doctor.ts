...
function resolveMode(cfg: MazelClawConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  intro("MazelClaw doctor");

  const root = await resolveMazelClawPackageRoot({
  ...
  let cfg: MazelClawConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("mazelclaw configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("mazelclaw config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("mazelclaw setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("mazelclaw doctor --fix")}" to apply changes.`);
  }
