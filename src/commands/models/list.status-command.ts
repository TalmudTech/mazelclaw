...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveMazelClawAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("mazelclaw models auth setup-token")}\` or \`${formatCliCommand("mazelclaw configure")}\`.`
          : `Run \`${formatCliCommand("mazelclaw configure")}\` or set an API key env var.`;
