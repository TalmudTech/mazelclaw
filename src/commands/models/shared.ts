...
export async function loadValidConfigOrThrow(): Promise<MazelClawConfig> {
...
export async function updateConfig(
  mutator: (cfg: MazelClawConfig) => MazelClawConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: MazelClawConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: MazelClawConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("mazelclaw agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
