...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under MAZELCLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("mazelclaw-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "mazelclaw-oauth-sync-"));
    ...
    process.env.MAZELCLAW_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "mazelclaw-oauth-nosync-"));
    process.env.MAZELCLAW_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside MAZELCLAW_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "mazelclaw-oauth-external-"));
    process.env.MAZELCLAW_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* MAZELCLAW_STATE_DIR
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to MAZELCLAW_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("mazelclaw-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
