...
describe("canvas host state dir defaults", () => {
  it("uses MAZELCLAW_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("mazelclaw-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("MazelClaw Canvas");
      } finally {
        await handler.close();
      }
    });
