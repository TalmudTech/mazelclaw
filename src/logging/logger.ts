...
import type { MazelClawConfig } from "../config/types.js";
...
export const DEFAULT_LOG_DIR = resolvePreferredMazelClawTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "mazelclaw.log"); // legacy single-file path

const LOG_PREFIX = "mazelclaw";
...
function resolveSettings(): ResolvedSettings {
  let cfg: MazelClawConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig?.("../config/config.js") as
        | {
            loadConfig?: () => MazelClawConfig;
          }
        | undefined;
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.MAZELCLAW_TEST_FILE_LOG !== "1" ? "silent" : "info";
