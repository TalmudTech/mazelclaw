...
import { resolveMazelClawAgentDir } from "../agents/agent-paths.js";
...
import { ensureMazelClawModelsJson } from "../agents/models-config.js";
import { discoverAuthStorage, discoverModels } from "../agents/pi-model-discovery.js";
import { loadConfig } from "../config/config.js";
import type { ModelsConfig, MazelClawConfig, ModelProviderConfig } from "../config/types.js";
...
const GATEWAY_LIVE = isTruthyEnvValue(process.env.MAZELCLAW_LIVE_GATEWAY);
const ZAI_FALLBACK = isTruthyEnvValue(process.env.MAZELCLAW_LIVE_GATEWAY_ZAI_FALLBACK);
const PROVIDERS = parseFilter(process.env.MAZELCLAW_LIVE_GATEWAY_PROVIDERS);
...
function sanitizeAuthConfig(params: {
  cfg: MazelClawConfig;
  agentDir: string;
}): MazelClawConfig["auth"] | undefined {
  ...
  let profiles: NonNullable<MazelClawConfig["auth"]>["profiles"] | undefined;
  if (auth.profiles) {
    profiles = {};
...
async function runGatewayModelSuite(params: GatewayModelSuiteParams) {
  const previous = {
    ...
    agentDir: process.env.MAZELCLAW_AGENT_DIR,
    piAgentDir: process.env.PI_CODING_AGENT_DIR,
    stateDir: process.env.MAZELCLAW_STATE_DIR,
  };
  ...
  const hostAgentDir = resolveMazelClawAgentDir();
  ...
  tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "mazelclaw-live-state-"));
  process.env.MAZELCLAW_STATE_DIR = tempStateDir;
  ...
  process.env.MAZELCLAW_AGENT_DIR = tempAgentDir;
  ...
  const toolProbePath = path.join(workspaceDir, `.mazelclaw-live-tool-probe.${nonceA}.txt`);
  await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);

  const agentDir = resolveMazelClawAgentDir();
  const sanitizedCfg: MazelClawConfig = {
    ...params.cfg,
  ...
  try {
        ...
        try {
          ...
          const toolProbe = await client.request<AgentFinalPayload>(
            "agent",
            {
              ...
              message:
                "MazelClaw live tool probe (local, safe): " +
    ...
    process.env.MAZELCLAW_AGENT_DIR = previous.agentDir;
    process.env.PI_CODING_AGENT_DIR = previous.piAgentDir;
    process.env.MAZELCLAW_STATE_DIR = previous.stateDir;
  }
}
...
describeLive("gateway live (dev agent, profile keys)", () => {
  it(
    ...
    async () => {
      ...
      await ensureMazelClawModelsJson(cfg);
      ...
      const rawModels = process.env.MAZELCLAW_LIVE_GATEWAY_MODELS?.trim();
  ...
  it("z.ai fallback handles anthropic tool history", async () => {
    ...
    const nonceA = randomUUID();
    const nonceB = randomUUID();
    const toolProbePath = path.join(workspaceDir, `.mazelclaw-live-zai-fallback.${nonceA}.txt`);
    await fs.writeFile(toolProbePath, `nonceA=${nonceA}\nnonceB=${nonceB}\n`);
