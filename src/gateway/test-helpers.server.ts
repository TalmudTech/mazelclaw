...
const GATEWAY_TEST_ENV_KEYS = [
  ...
  "MAZELCLAW_SKIP_BROWSER_CONTROL_SERVER",
  "MAZELCLAW_SKIP_GMAIL_WATCHER",
  "MAZELCLAW_SKIP_CANVAS_HOST",
  "MAZELCLAW_BUNDLED_PLUGINS_DIR",
  "MAZELCLAW_SKIP_CHANNELS",
  "MAZELCLAW_SKIP_PROVIDERS",
  "MAZELCLAW_SKIP_CRON",
  "MAZELCLAW_TEST_MINIMAL_GATEWAY",
] as const;
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "mazelclaw-gateway-home-"));
...
function applyGatewaySkipEnv() {
  process.env.MAZELCLAW_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.MAZELCLAW_SKIP_GMAIL_WATCHER = "1";
  process.env.MAZELCLAW_SKIP_CANVAS_HOST = "1";
  process.env.MAZELCLAW_SKIP_CHANNELS = "1";
  process.env.MAZELCLAW_SKIP_PROVIDERS = "1";
  process.env.MAZELCLAW_SKIP_CRON = "1";
  process.env.MAZELCLAW_TEST_MINIMAL_GATEWAY = "1";
  process.env.MAZELCLAW_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "mazelclaw-test-no-bundled-extensions")
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    tempConfigRoot = await fs.mkdtemp(path.join(tempHome, "mazelclaw-test-"));
  } else {
    tempConfigRoot = path.join(tempHome, ".mazelclaw-test");
...
export async function startServerWithClient(
  ...
  const envSnapshot = captureEnv(["MAZELCLAW_GATEWAY_TOKEN"]);
  const prev = process.env.MAZELCLAW_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.MAZELCLAW_GATEWAY_TOKEN = fallbackToken;
  }
  ...
  const defaultPassword =
    opts?.skipDefaultAuth === true
      ...
      : typeof (testState.gatewayAuth as { password?: unknown } | undefined)?.password === "string"
        ...
        : process.env.MAZELCLAW_GATEWAY_PASSWORD;
