    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "mazelclaw", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-mazelclaw-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        await expectAgentSessionKeyMatch({
          ...
          headers: { "x-mazelclaw-agent-id": "alpha" },
          matcher: /^agent:alpha:/,
        });
      }

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "mazelclaw", messages: [{ role: "user", content: "hi" }] },
          {
            "x-mazelclaw-agent-id": "beta",
            "x-mazelclaw-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
