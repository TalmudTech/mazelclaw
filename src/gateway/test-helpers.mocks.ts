...
type GetReplyFromConfigFn = (
  ...
  configOverride?: MazelClawConfig,
...
const testConfigRoot = {
  value: path.join(os.tmpdir(), `mazelclaw-gateway-test-${process.pid}-${crypto.randomUUID()}`),
};

export const setTestConfigRoot = (root: string) => {
  testConfigRoot.value = root;
  process.env.MAZELCLAW_CONFIG_PATH = path.join(root, "mazelclaw.json");
};
...
vi.mock("../config/config.js", async () => {
  ...
  const resolveConfigPath = () => path.join(testConfigRoot.value, "mazelclaw.json");
  ...
  return {
    ...
    loadConfig: () => {
      ...
      const defaults = {
        ...
        workspace: path.join(os.tmpdir(), "mazelclaw-gateway-test"),
        ...fileDefaults,
...
vi.mock("../plugins/loader.js", async () => {
  ...
  return {
    ...actual,
    loadMazelClawPlugins: () => pluginRegistryState.registry,
  };
});
