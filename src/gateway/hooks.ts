...
export function resolveHooksConfig(cfg: MazelClawConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: MazelClawConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-mazelclaw-token"] === "string"
      ? req.headers["x-mazelclaw-token"].trim()
      : "";
  if (headerToken) {
    return headerToken;
  }
