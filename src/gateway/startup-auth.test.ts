...
const mocks = vi.hoisted(() => ({
  writeConfigFile: vi.fn(async (_cfg: MazelClawConfig) => {}),
}));
...
describe("ensureGatewayStartupAuth", () => {
  async function expectEphemeralGeneratedTokenWhenOverridden(cfg: MazelClawConfig) {
  ...
  it("throws when hooks token reuses gateway token resolved from env", async () => {
    await expect(
      ensureGatewayStartupAuth({
        ...
        env: {
          MAZELCLAW_GATEWAY_TOKEN: "shared-gateway-token-1234567890",
