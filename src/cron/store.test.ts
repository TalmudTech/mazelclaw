...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "mazelclaw-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses MAZELCLAW_HOME for tilde expansion", () => {
    vi.stubEnv("MAZELCLAW_HOME", "/srv/mazelclaw-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/mazelclaw-home", "cron", "jobs.json"));
  });
});
