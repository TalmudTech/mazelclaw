...
export async function withTempCronHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "mazelclaw-cron-" });
}
...
export async function writeSessionStore(
  ...
  const dir = path.join(home, ".mazelclaw", "sessions");
...
export function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<MazelClawConfig> = {},
): MazelClawConfig {
  const base: MazelClawConfig = {
    agents: {
      defaults: {
