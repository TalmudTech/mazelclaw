...
function createReplyConfig(home: string, streamMode?: "block"): MazelClawConfig {
  return {
    agents: {
      defaults: {
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeHarness("mazelclaw-stream-", async (home) => {
...
describe("block streaming", () => {
  beforeEach(() => {
    vi.stubEnv("MAZELCLAW_TEST_FAST", "1");
