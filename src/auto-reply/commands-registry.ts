...
export function isCommandEnabled(cfg: MazelClawConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: MazelClawConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as MazelClawConfig),
...
export function getCommandDetection(_cfg?: MazelClawConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: MazelClawConfig) {
