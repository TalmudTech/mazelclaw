...
function resolveVoiceTtsConfig(params: { cfg: MazelClawConfig; override?: TtsConfig }): {
  cfg: MazelClawConfig;
...
async function writeWavFile(pcm: Buffer): Promise<{ path: string; durationSeconds: number }> {
  const tempDir = await fs.mkdtemp(path.join(resolvePreferredMazelClawTmpDir(), "discord-voice-"));
