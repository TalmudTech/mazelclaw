...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    await withTempDirSync("mazelclaw-test-", async (tmp) => {
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    withTempDirSync("mazelclaw-auth-", (authDir) => {
  ...
  it("falls back through lidMappingDirs in order", () => {
    withTempDirSync("mazelclaw-lid-a-", (first) => {
      withTempDirSync("mazelclaw-lid-b-", (second) => {
...
describe("resolveConfigDir", () => {
  it("prefers ~/.mazelclaw when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "mazelclaw-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers MAZELCLAW_HOME over HOME", () => {
    vi.stubEnv("MAZELCLAW_HOME", "/srv/mazelclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/mazelclaw-home"));
...
describe("shortenHomePath", () => {
  it("uses $MAZELCLAW_HOME prefix when MAZELCLAW_HOME is set", () => {
    vi.stubEnv("MAZELCLAW_HOME", "/srv/mazelclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/mazelclaw-home")}/.mazelclaw/mazelclaw.json`)).toBe(
      "$MAZELCLAW_HOME/.mazelclaw/mazelclaw.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $MAZELCLAW_HOME replacement when MAZELCLAW_HOME is set", () => {
    vi.stubEnv("MAZELCLAW_HOME", "/srv/mazelclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/mazelclaw-home")}/.mazelclaw/mazelclaw.json`),
    ).toBe("config: $MAZELCLAW_HOME/.mazelclaw/mazelclaw.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/mazelclaw")).toBe(path.resolve(os.homedir(), "mazelclaw"));
  });
  ...
  it("prefers MAZELCLAW_HOME for tilde expansion", () => {
    vi.stubEnv("MAZELCLAW_HOME", "/srv/mazelclaw-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/mazelclaw")).toBe(path.resolve("/srv/mazelclaw-home", "mazelclaw"));
