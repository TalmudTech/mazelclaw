...
function resolveExecConfig(params: { cfg?: MazelClawConfig; agentId?: string }) {
...
function resolveFsConfig(params: { cfg?: MazelClawConfig; agentId?: string }) {
  const cfg = params.cfg;
...
export function createMazelClawCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createMazelClawReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
