...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.MAZELCLAW_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".mazelclaw", `workspace-${profile}`);
  }
  return path.join(home, ".mazelclaw", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".mazelclaw";
