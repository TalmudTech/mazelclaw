...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses MAZELCLAW_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      MAZELCLAW_HOME: "/srv/mazelclaw-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/mazelclaw-home"), ".mazelclaw", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".mazelclaw", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("mazelclaw-workspace-");
