...
const defaultTools = createMazelClawCodingTools({ senderIsOwner: true });
...
describe("createMazelClawCodingTools", () => {
  ...
  it("gates apply_patch behind tools.exec.applyPatch for OpenAI models", () => {
    ...
    const openAiTools = createMazelClawCodingTools({
      config,
    ...
    const anthropicTools = createMazelClawCodingTools({
      config,
  ...
  it("respects apply_patch allowModels", () => {
    ...
    const allowed = createMazelClawCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5.2",
    });
    ...
    const denied = createMazelClawCodingTools({
      config,
      modelProvider: "openai",
      modelId: "gpt-5-mini",
    });
