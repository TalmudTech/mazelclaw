...
type WebSearchConfig = NonNullable<MazelClawConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: MazelClawConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("mazelclaw configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.mazelclaw.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://mazelclaw.ai",
      "X-Title": "MazelClaw Web Search",
    },
