...
} from "./mazelclaw-tools.subagents.sessions-spawn.test-harness.js";
...
type CreateMazelClawTools = (typeof import("./mazelclaw-tools.js"))["createMazelClawTools"];
type CreateMazelClawToolsOpts = Parameters<CreateMazelClawTools>[0];

async function getSessionsSpawnTool(opts: CreateMazelClawToolsOpts) {
  // Dynamic import: ensure harness mocks are installed before tool modules load.
  const { createMazelClawTools } = await import("./mazelclaw-tools.js");
  const tool = createMazelClawTools(opts).find((candidate) => candidate.name === "sessions_spawn");
  if (!tool) {
...
describe("mazelclaw-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
  });
