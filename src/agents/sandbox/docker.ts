...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "mazelclaw.sandbox=1");
  args.push("--label", `mazelclaw.sessionKey=${params.scopeKey}`);
  args.push("--label", `mazelclaw.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `mazelclaw.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "mazelclaw.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`mazelclaw sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`mazelclaw sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("mazelclaw sandbox recreate --all");
}
