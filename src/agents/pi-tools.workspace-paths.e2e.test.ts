...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("mazelclaw-ws-", async (workspaceDir) => {
      await withTempDir("mazelclaw-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createMazelClawCodingTools({ workspaceDir });
    ...
    await withTempDir("mazelclaw-ws-", async (workspaceDir) => {
      await withTempDir("mazelclaw-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello mazelclaw");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("mazelclaw-ws-", async (workspaceDir) => {
      await withTempDir("mazelclaw-override-", async (overrideDir) => {
        const tools = createMazelClawCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("mazelclaw-sandbox-", async (sandboxDir) => {
      await withTempDir("mazelclaw-workspace-", async (workspaceDir) => {
        ...
        const tools = createMazelClawCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
