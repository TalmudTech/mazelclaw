...
describe("createMazelClawCodingTools", () => {
  it("keeps read tool image metadata intact", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "mazelclaw-read-"));
    try {
  ...
  it("returns text content without image blocks for text files", async () => {
    ...
    try {
      ...
      const contents = "Hello from mazelclaw read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    ...
    const tools = createMazelClawCodingTools({ sandbox });
