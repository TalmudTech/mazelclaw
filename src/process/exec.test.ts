...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    await withEnvAsync({ MAZELCLAW_BASE_ENV: "base" }, async () => {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.MAZELCLAW_BASE_ENV ?? "") + "|" + (process.env.MAZELCLAW_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { MAZELCLAW_TEST_ENV: "ok" },
        },
      );
