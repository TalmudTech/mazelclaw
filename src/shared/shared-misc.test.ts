...
describe("shared/frontmatter", () => {
  ...
  test("resolveMazelClawManifestBlock parses JSON5 metadata and picks mazelclaw block", () => {
    const frontmatter = {
      metadata: "{ mazelclaw: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveMazelClawManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveMazelClawManifestBlock returns undefined for invalid input", () => {
    expect(resolveMazelClawManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(
      resolveMazelClawManifestBlock({ frontmatter: { metadata: "not-json5" } }),
    ).toBeUndefined();
    expect(
      resolveMazelClawManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
