  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          mazelclaw: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.MAZELCLAW_GATEWAY_PORT;
    process.env.MAZELCLAW_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.MAZELCLAW_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.MAZELCLAW_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (prevGatewayPassword === undefined) {
      ...
      process.env.MAZELCLAW_GATEWAY_PASSWORD = prevGatewayPassword;
    }
