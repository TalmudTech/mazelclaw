...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredMazelClawTmpDir: vi.fn(() => "/tmp/mazelclaw"),
}));
vi.mock("../infra/tmp-mazelclaw-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredMazelClawTmpDir.mockReturnValue("/tmp/mazelclaw");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredMazelClawTmpDir.mockReturnValue("/tmp/mazelclaw-preferred");
    ...
    const expectedDownloadsTail = `${path.join("tmp", "mazelclaw-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredMazelClawTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "mazelclaw-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "mazelclaw-preferred", "downloads")}${path.sep}`),
    );
  });
