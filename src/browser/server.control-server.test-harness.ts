...
export function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.MAZELCLAW_GATEWAY_PORT = prevGatewayPort;
}
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/mazelclaw" }));
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "mazelclaw",
        profiles: {
          mazelclaw: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchMazelClawChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
...
export function installBrowserControlServerHooks() {
  beforeEach(async () => {
    ...
    state.prevGatewayPort = process.env.MAZELCLAW_GATEWAY_PORT;
    process.env.MAZELCLAW_GATEWAY_PORT = String(state.testPort - 2);
    ...
    state.prevGatewayToken = process.env.MAZELCLAW_GATEWAY_TOKEN;
    state.prevGatewayPassword = process.env.MAZELCLAW_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (state.prevGatewayToken === undefined) {
      ...
      process.env.MAZELCLAW_GATEWAY_TOKEN = state.prevGatewayToken;
    }
    if (state.prevGatewayPassword === undefined) {
      delete process.env.MAZELCLAW_GATEWAY_PASSWORD;
    } else {
      process.env.MAZELCLAW_GATEWAY_PASSWORD = state.prevGatewayPassword;
    }
