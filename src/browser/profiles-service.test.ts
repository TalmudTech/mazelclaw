...
vi.mock("./chrome.js", () => ({
  resolveMazelClawUserDataDir: vi.fn(() => "/tmp/mazelclaw-test/mazelclaw/user-data"),
}));

import { loadConfig, writeConfigFile } from "../config/config.js";
import { resolveMazelClawUserDataDir } from "./chrome.js";
...
describe("BrowserProfilesService", () => {
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "mazelclaw-profile-"));
    const userDataDir = path.join(tempDir, "work", "user-data");
    fs.mkdirSync(path.dirname(userDataDir), { recursive: true });
    vi.mocked(resolveMazelClawUserDataDir).mockReturnValue(userDataDir);
