import type { BrowserConfig, BrowserProfileConfig, MazelClawConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_MAZELCLAW_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "mazelclaw" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_MAZELCLAW_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "mazelclaw" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_MAZELCLAW_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_MAZELCLAW_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an MazelClaw browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for mazelclaw/mazelclaw CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: MazelClawConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_MAZELCLAW_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "mazelclaw";
