...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".mazelclaw-profile-decorated");
}
 ...
 */
export function decorateMazelClawProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_MAZELCLAW_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_MAZELCLAW_BROWSER_COLOR).toUpperCase();
