...
vi.mock("./mazelclaw-root.js", () => ({
  resolveMazelClawPackageRoot: vi.fn(async () => null),
  resolveMazelClawPackageRootSync: vi.fn(() => null),
}));
...
let mazelclawRoot: typeof import("./mazelclaw-root.js");
...
describe("control UI assets helpers (fs-mocked)", () => {
  beforeAll(async () => {
    ...
    mazelclawRoot = await import("./mazelclaw-root.js");
  });
  ...
  it("uses resolveMazelClawPackageRoot when available", async () => {
    const pkgRoot = abs("fixtures/mazelclaw");
    (
      mazelclawRoot.resolveMazelClawPackageRoot as unknown as ReturnType<typeof vi.fn>
    ).mockResolvedValueOnce(pkgRoot);

    await expect(resolveControlUiDistIndexPath(abs("fixtures/bin/mazelclaw"))).resolves.toBe(
  ...
  it("falls back to package.json name matching when root resolution fails", async () => {
    ...
    setFile(path.join(root, "package.json"), JSON.stringify({ name: "mazelclaw" }));
    setFile(path.join(root, "dist", "control-ui", "index.html"), "<html></html>\n");

    await expect(resolveControlUiDistIndexPath(path.join(root, "mazelclaw.mjs"))).resolves.toBe(
      path.join(root, "dist", "control-ui", "index.html"),
    );
  });

  it("returns null when fallback package name does not match", async () => {
    const root = abs("fixtures/not-mazelclaw");
  ...
  it("resolves control-ui root for dist bundle argv1 and moduleUrl candidates", async () => {
    const pkgRoot = abs("fixtures/mazelclaw-bundle");
    (
      mazelclawRoot.resolveMazelClawPackageRootSync as unknown as ReturnType<typeof vi.fn>
