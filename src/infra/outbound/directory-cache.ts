...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: MazelClawConfig | null = null;
  ...
  get(key: string, cfg: MazelClawConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: MazelClawConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  clear(cfg?: MazelClawConfig): void {
    this.cache.clear();
    if (cfg) {
  ...
  private resetIfConfigChanged(cfg: MazelClawConfig): void {
