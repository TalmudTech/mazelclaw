...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__mazelclaw_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "mazelclaw-root");
...
describe("resolveMazelClawPackageRoot", () => {
  let resolveMazelClawPackageRoot: typeof import("./mazelclaw-root.js").resolveMazelClawPackageRoot;
  let resolveMazelClawPackageRootSync: typeof import("./mazelclaw-root.js").resolveMazelClawPackageRootSync;

  beforeAll(async () => {
    ({ resolveMazelClawPackageRoot, resolveMazelClawPackageRootSync } =
      await import("./mazelclaw-root.js"));
  });
  ...
  it("resolves package root from .bin argv1", async () => {
    ...
    const argv1 = path.join(project, "node_modules", ".bin", "mazelclaw");
    const pkgRoot = path.join(project, "node_modules", "mazelclaw");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "mazelclaw" }));

    expect(resolveMazelClawPackageRootSync({ argv1 })).toBe(pkgRoot);
  });

  it("resolves package root via symlinked argv1", async () => {
    const project = fx("symlink-scenario");
    const bin = path.join(project, "bin", "mazelclaw");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "mazelclaw.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "mazelclaw" }));

    expect(resolveMazelClawPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveMazelClawPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-mazelclaw package roots", async () => {
    const pkgRoot = fx("not-mazelclaw");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-mazelclaw" }));

    expect(resolveMazelClawPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });
  ...
  it("async resolver matches sync behavior", async () => {
    ...
    await expect(resolveMazelClawPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });

  it("async resolver returns null when no package roots exist", async () => {
    await expect(resolveMazelClawPackageRoot({ cwd: fx("missing") })).resolves.toBeNull();
  });
});
