...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.MAZELCLAW_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.MAZELCLAW_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set MAZELCLAW_APNS_TEAM_ID and MAZELCLAW_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.MAZELCLAW_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.MAZELCLAW_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.MAZELCLAW_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set MAZELCLAW_APNS_PRIVATE_KEY_P8 or MAZELCLAW_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading MAZELCLAW_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
...
function createMazelClawPushMetadata(params: {
...
export async function sendApnsAlert(params: {
  ...
  const payload = {
    ...
    mazelclaw: createMazelClawPushMetadata({
      kind: "push.test",
      nodeId: params.nodeId,
    }),
  };
