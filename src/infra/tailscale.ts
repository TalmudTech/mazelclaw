...
export async function getTailscaleBinary(): Promise<string> {
  const forcedBinary = process.env.MAZELCLAW_TEST_TAILSCALE_BINARY?.trim();
  if (forcedBinary) {
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for MazelClaw. You can keep running the web gateway without it: \`${formatCliCommand("mazelclaw gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
