...
const DEFAULT_PACKAGE_NAME = "mazelclaw";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "mazelclaw-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "mazelclaw.mjs");
    ...
    const doctorStep = await runStep(
      step("mazelclaw doctor", doctorArgv, gitRoot, { MAZELCLAW_UPDATE_IN_PROGRESS: "1" }),
    );
