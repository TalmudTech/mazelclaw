...
import { POSIX_MAZELCLAW_TMP_DIR, resolvePreferredMazelClawTmpDir } from "./tmp-mazelclaw-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredMazelClawTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `mazelclaw-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredMazelClawTmpDir({
    accessSync,
...
describe("resolvePreferredMazelClawTmpDir", () => {
  it("prefers /tmp/mazelclaw when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_MAZELCLAW_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/mazelclaw when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_MAZELCLAW_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/mazelclaw when /tmp/mazelclaw is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/mazelclaw when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/mazelclaw is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/mazelclaw is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/mazelclaw is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
