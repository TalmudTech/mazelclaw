...
function getDiagnosticEventsState(): DiagnosticEventsGlobalState {
  const globalStore = globalThis as typeof globalThis & {
    __mazelclawDiagnosticEventsState?: DiagnosticEventsGlobalState;
  };
  if (!globalStore.__mazelclawDiagnosticEventsState) {
    globalStore.__mazelclawDiagnosticEventsState = {
      seq: 0,
      listeners: new Set<(evt: DiagnosticEventPayload) => void>(),
      dispatchDepth: 0,
    };
  }
  return globalStore.__mazelclawDiagnosticEventsState;
}

export function isDiagnosticsEnabled(config?: MazelClawConfig): boolean {
