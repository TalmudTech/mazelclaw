...
function candidateBinDirs(opts: EnsureMazelClawPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `mazelclaw` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "mazelclaw");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.MAZELCLAW_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "mazelclaw"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `mazelclaw` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureMazelClawCliOnPath(opts: EnsureMazelClawPathOpts = {}) {
  if (isTruthyEnvValue(process.env.MAZELCLAW_PATH_BOOTSTRAPPED)) {
    return;
  }
