...
export function triggerMazelClawRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.MAZELCLAW_SYSTEMD_UNIT,
        process.env.MAZELCLAW_PROFILE,
      );
  ...
  const label =
    process.env.MAZELCLAW_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.MAZELCLAW_PROFILE);
