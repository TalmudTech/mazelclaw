...
const TEMP_DIR_PREFIX = "mazelclaw-install-source-utils-";
...
describe("withTempDir", () => {
  it("creates a temp dir and always removes it after callback", async () => {
    ...
    const value = await withTempDir("mazelclaw-install-source-utils-", async (tmpDir) => {
      observedDir = tmpDir;
...
describe("resolveArchiveSourcePath", () => {
  it("returns not found error for missing archive paths", async () => {
    const result = await resolveArchiveSourcePath("/tmp/does-not-exist-mazelclaw-archive.tgz");
...
describe("packNpmSpecToArchive", () => {
  it("packs spec and returns archive path using JSON output metadata", async () => {
    ...
    mockPackCommandResult({
      stdout: JSON.stringify([
        {
          id: "mazelclaw-plugin@1.2.3",
          name: "mazelclaw-plugin",
          version: "1.2.3",
          filename: "mazelclaw-plugin-1.2.3.tgz",
    ...
    const result = await runPack("mazelclaw-plugin@1.2.3", cwd);

    expect(result).toEqual({
      ok: true,
      archivePath: path.join(cwd, "mazelclaw-plugin-1.2.3.tgz"),
      metadata: {
        name: "mazelclaw-plugin",
        version: "1.2.3",
        resolvedSpec: "mazelclaw-plugin@1.2.3",
        integrity: "sha512-test-integrity",
        shasum: "abc123",
      },
    });
    expect(runCommandWithTimeoutMock).toHaveBeenCalledWith(
      ["npm", "pack", "mazelclaw-plugin@1.2.3", "--ignore-scripts", "--json"],
  ...
  it("returns explicit error when npm pack produces no archive name", async () => {
    ...
    const result = await runPack("mazelclaw-plugin@1.2.3", cwd, 5000);
  ...
  it("parses scoped metadata from id-only json output even with npm notice prefix", async () => {
    ...
    mockPackCommandResult({
      stdout:
        ...
        JSON.stringify([
          {
            id: "@mazelclaw/plugin-demo@2.0.0",
            filename: "mazelclaw-plugin-demo-2.0.0.tgz",
          },
        ]),
    });

    const result = await runPack("@mazelclaw/plugin-demo@2.0.0", cwd);
    expect(result).toEqual({
      ok: true,
      archivePath: path.join(cwd, "mazelclaw-plugin-demo-2.0.0.tgz"),
      metadata: {
        resolvedSpec: "@mazelclaw/plugin-demo@2.0.0",
      },
    });
  });
