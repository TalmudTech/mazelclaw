...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: MazelClawConfig) => void;
};

function hasExplicitHeartbeatAgents(cfg: MazelClawConfig) {
  const list = cfg.agents?.list ?? [];
  return list.some((entry) => Boolean(entry?.heartbeat));
}

export function isHeartbeatEnabledForAgent(cfg: MazelClawConfig, agentId?: string): boolean {
...
function resolveHeartbeatAgents(cfg: MazelClawConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
export function resolveHeartbeatPrompt(cfg: MazelClawConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt);
}

function resolveHeartbeatAckMaxChars(cfg: MazelClawConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: MazelClawConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
