...
vi.mock("./install-source-utils.js", async (importOriginal) => {
  ...
  return {
    ...
    withTempDir: vi.fn(async (_prefix: string, fn: (tmpDir: string) => Promise<unknown>) => {
      return await fn("/tmp/mazelclaw-npm-pack-install-test");
    }),
...
describe("installFromNpmSpecArchive", () => {
  ...
  it("returns pack errors without invoking installer", async () => {
    ...
    const result = await installFromNpmSpecArchive({
      tempDirPrefix: "mazelclaw-test-",
      spec: "@mazelclaw/test@1.0.0",
      timeoutMs: 1000,
      installFromArchive,
    });
    ...
    expect(withTempDir).toHaveBeenCalledWith("mazelclaw-test-", expect.any(Function));
  });

  it("returns resolution metadata and installer result on success", async () => {
    vi.mocked(packNpmSpecToArchive).mockResolvedValue({
      ok: true,
      archivePath: "/tmp/mazelclaw-test.tgz",
      metadata: {
        name: "@mazelclaw/test",
        version: "1.0.0",
        resolvedSpec: "@mazelclaw/test@1.0.0",
    ...
    expect(result.npmResolution.resolvedSpec).toBe("@mazelclaw/test@1.0.0");
    expect(result.npmResolution.resolvedAt).toBeTruthy();
    expect(installFromArchive).toHaveBeenCalledWith({ archivePath: "/tmp/mazelclaw-test.tgz" });
  });
  ...
  it("aborts when integrity drift callback rejects drift", async () => {
    ...
    expect(result).toEqual({
      ok: false,
      error: "aborted: npm package integrity drift detected for @mazelclaw/test@1.0.0",
    });
    expect(installFromArchive).not.toHaveBeenCalled();
  });
  ...
  it("warns and proceeds on drift when no callback is configured", async () => {
    ...
    expect(warn).toHaveBeenCalledWith(
      "Integrity drift detected for @mazelclaw/test@1.0.0: expected sha512-old, got sha512-new",
    );
  });
  ...
  it("returns installer failures to callers for domain-specific handling", async () => {
    vi.mocked(packNpmSpecToArchive).mockResolvedValue({
      ok: true,
      archivePath: "/tmp/mazelclaw-test.tgz",
      metadata: { resolvedSpec: "@mazelclaw/test@1.0.0", integrity: "sha512-same" },
    });
