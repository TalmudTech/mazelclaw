...
describe("fs-safe", () => {
  it("reads a local file safely", async () => {
    const dir = await makeTempDir("mazelclaw-fs-safe-");
  ...
  it("blocks traversal outside root", async () => {
    const root = await makeTempDir("mazelclaw-fs-safe-root-");
    const outside = await makeTempDir("mazelclaw-fs-safe-outside-");
