...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.MAZELCLAW_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*mazelclaw-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "mazelclaw";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "mazelclaw");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "mazelclaw-gw");
  ...
  records.push(`_mazelclaw-gw._tcp IN PTR ${instanceLabel}._mazelclaw-gw._tcp`);
  records.push(`${instanceLabel}._mazelclaw-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._mazelclaw-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; mazelclaw-content-hash: ${contentHash}\n${contentBody}`;
}
