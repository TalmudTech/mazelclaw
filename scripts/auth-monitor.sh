...
#
# Suggested cron: */30 * * * * /home/admin/mazelclaw/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send MazelClaw notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., mazelclaw-alerts)
...
STATE_FILE="$HOME/.mazelclaw/auth-monitor-state"
...
send_notification() {
    ...
    # Send via MazelClaw if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use mazelclaw
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via MazelClaw to $NOTIFY_PHONE..."
            mazelclaw send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! MazelClaw is down. Run: ssh l36 '~/mazelclaw/scripts/mobile-reauth.sh'" "urgent"
    exit 1
