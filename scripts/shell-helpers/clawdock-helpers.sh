#!/usr/bin/env bash
# ClawDock - Docker helpers for MazelClaw
# Inspired by Simon Willison's "Running MazelClaw in Docker"
# https://til.simonwillison.net/llms/mazelclaw-docker
#
# Installation:
...
# Common paths to check for MazelClaw
CLAWDOCK_COMMON_PATHS=(
  "${HOME}/mazelclaw"
  "${HOME}/workspace/mazelclaw"
  "${HOME}/projects/mazelclaw"
  "${HOME}/dev/mazelclaw"
  "${HOME}/code/mazelclaw"
  "${HOME}/src/mazelclaw"
)
...
_clawdock_ensure_dir() {
  ...
  if [[ -n "$found_path" ]]; then
    echo ""
    echo "🦞 Found MazelClaw at: $found_path"
    echo -n "   Use this location? [Y/n] "
    read -r response
    if [[ "$response" =~ ^[Nn] ]]; then
      echo ""
      echo "Set CLAWDOCK_DIR manually:"
      echo "  export CLAWDOCK_DIR=/path/to/mazelclaw"
      return 1
    fi
    CLAWDOCK_DIR="$found_path"
  else
    echo ""
    echo "❌ MazelClaw not found in common locations."
    echo ""
    echo "Clone it first:"
    echo ""
    echo "  git clone https://github.com/mazelclaw/mazelclaw.git ~/mazelclaw"
    echo "  cd ~/mazelclaw && ./docker-setup.sh"
    echo ""
...
_clawdock_read_env_token() {
  ...
  local raw
  raw=$(sed -n 's/^MAZELCLAW_GATEWAY_TOKEN=//p' "${CLAWDOCK_DIR}/.env" | head -n 1)
  if [[ -z "$raw" ]]; then
    return 1
  fi
  _clawdock_trim_quotes "$raw"
...
# Basic Operations
clawdock-start() {
  _clawdock_compose up -d mazelclaw-gateway
}
...
clawdock-restart() {
  _clawdock_compose restart mazelclaw-gateway
}

clawdock-logs() {
  _clawdock_compose logs -f mazelclaw-gateway
}
...
clawdock-workspace() {
  cd ~/.mazelclaw/workspace
}

# Container Access
clawdock-shell() {
  _clawdock_compose exec mazelclaw-gateway \
    bash -c 'echo "alias mazelclaw=\"./mazelclaw.mjs\"" > /tmp/.bashrc_mazelclaw && bash --rcfile /tmp/.bashrc_mazelclaw'
}
...
clawdock-cli() {
  _clawdock_compose run --rm mazelclaw-cli "$@"
}

# Maintenance
clawdock-rebuild() {
  _clawdock_compose build mazelclaw-gateway
}
...
clawdock-health() {
  ...
  _clawdock_compose exec -e "MAZELCLAW_GATEWAY_TOKEN=$token" mazelclaw-gateway \
...
clawdock-fix-token() {
  ...
  _clawdock_compose exec -e "TOKEN=$token" mazelclaw-gateway \
    bash -c './mazelclaw.mjs config set gateway.remote.token "$TOKEN" && ./mazelclaw.mjs config set gateway.auth.token "$TOKEN"' 2>&1 | _clawdock_filter_warnings

  echo "🔍 Verifying token was saved..."
  local saved_token
  saved_token=$(_clawdock_compose exec mazelclaw-gateway \
    bash -c "./mazelclaw.mjs config get gateway.remote.token 2>/dev/null" 2>&1 | _clawdock_filter_warnings | tr -d '\r\n' | head -c 64)
  ...
  _clawdock_compose restart mazelclaw-gateway 2>&1 | _clawdock_filter_warnings
...
clawdock-dashboard() {
  ...
  output=$(_clawdock_compose run --rm mazelclaw-cli dashboard --no-open 2>&1)
  exit_status=$?
...
clawdock-devices() {
  ...
  output=$(_clawdock_compose exec mazelclaw-gateway node dist/index.js devices list 2>&1)
  exit_status=$?
  ...
  if [ $exit_status -ne 0 ]; then
    ...
    echo -e "      $(_cmd 'mazelclaw config get gateway.remote.token')"
    return 1
  fi
...
clawdock-help() {
  echo -e "\n${_CLR_BOLD}${_CLR_CYAN}🦞 ClawDock - Docker Helpers for MazelClaw${_CLR_RESET}\n"
  ...
  echo -e "  $(_cmd clawdock-shell)       ${_CLR_DIM}Shell into container (mazelclaw alias ready)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-cd)          ${_CLR_DIM}Jump to mazelclaw project directory${_CLR_RESET}"
  echo -e "  $(_cmd clawdock-config)      ${_CLR_DIM}Open config directory (~/.mazelclaw)${_CLR_RESET}"
  ...
  echo -e "  $(_cmd clawdock-shell)"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'mazelclaw channels login --channel whatsapp')"
  echo -e "    ${_CLR_BLUE}>${_CLR_RESET} $(_cmd 'mazelclaw status')"
  echo ""
  ...
  echo -e "${_CLR_BLUE}📚 Docs: ${_CLR_RESET}${_CLR_CYAN}https://docs.mazelclaw.ai${_CLR_RESET}"
  echo ""
}
