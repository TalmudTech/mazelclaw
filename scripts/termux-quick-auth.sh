...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/mazelclaw/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "MazelClaw Re-Auth" -c "After getting key, run: ssh $SERVER '~/mazelclaw/scripts/mobile-reauth.sh'" --id mazelclaw-auth
        ;;
    *)
