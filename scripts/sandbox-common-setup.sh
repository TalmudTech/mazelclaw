...
BASE_IMAGE="${BASE_IMAGE:-mazelclaw-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-mazelclaw-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=mazelclaw.sandbox=1)
NOTE
