...
INSTALL_URL="${MAZELCLAW_INSTALL_URL:-https://mazelclaw.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${MAZELCLAW_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${MAZELCLAW_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="mazelclaw"
PACKAGE_NAME="${MAZELCLAW_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (MAZELCLAW_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${MAZELCLAW_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${MAZELCLAW_INSTALL_LATEST_OUT:-}"
fi
if [[ -n "$CMD_PATH" ]]; then
