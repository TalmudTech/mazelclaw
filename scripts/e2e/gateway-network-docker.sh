...
IMAGE_NAME="mazelclaw-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="mazelclaw-net-e2e-$$"
GW_NAME="mazelclaw-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "MAZELCLAW_GATEWAY_TOKEN=$TOKEN" \
	  -e "MAZELCLAW_SKIP_CHANNELS=1" \
	  -e "MAZELCLAW_SKIP_GMAIL_WATCHER=1" \
	  -e "MAZELCLAW_SKIP_CRON=1" \
	  -e "MAZELCLAW_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
