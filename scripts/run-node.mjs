...
const shouldBuild = (deps) => {
  if (deps.env.MAZELCLAW_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.MAZELCLAW_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[mazelclaw] ${message}\n`);
};

const runMazelClaw = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["mazelclaw.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
...
export async function runNodeMain(params = {}) {
  ...
  if (!shouldBuild(deps)) {
    return await runMazelClaw(deps);
  }
