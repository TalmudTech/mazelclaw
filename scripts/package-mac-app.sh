...
# Build and bundle MazelClaw into a minimal .app we can open.
# Outputs to dist/MazelClaw.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/MazelClaw.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="MazelClaw"
BUNDLE_ID="${BUNDLE_ID:-ai.mazelclaw.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/mazelclaw/mazelclaw/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/MazelClaw/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :MazelClawBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :MazelClawGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/MazelClaw"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/MazelClaw"
fi
chmod +x "$APP_ROOT/Contents/MacOS/MazelClaw"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/MazelClaw" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/MazelClaw/Resources/MazelClaw.icns" "$APP_ROOT/Contents/Resources/MazelClaw.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/MazelClaw/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying MazelClawKit resources"
MAZELCLAWKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/MazelClawKit_MazelClawKit.bundle"
if [ -d "$MAZELCLAWKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/MazelClawKit_MazelClawKit.bundle"
  cp -R "$MAZELCLAWKIT_BUNDLE" "$APP_ROOT/Contents/Resources/MazelClawKit_MazelClawKit.bundle"
else
  echo "WARN: MazelClawKit resource bundle not found at $MAZELCLAWKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running MazelClaw"
killall -q MazelClaw 2>/dev/null || true
