...
ZIP=${1:?"Usage: $0 MazelClaw-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/mazelclaw/mazelclaw/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^MazelClaw-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/mazelclaw/mazelclaw/releases/download/v${VERSION}/"}
