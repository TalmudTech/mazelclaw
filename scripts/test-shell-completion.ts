 ...
 * `mazelclaw update`. Use it to verify the completion installation flow
...
\`mazelclaw update\`, \`mazelclaw doctor\`, and \`mazelclaw onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(
      theme.muted("  1. Remove the '# MazelClaw Completion' block from your shell profile"),
    );
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`mazelclaw completion --install\` later to enable.`));
    return;
  }
