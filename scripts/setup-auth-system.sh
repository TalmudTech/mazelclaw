#!/bin/bash
# Setup MazelClaw Auth Management System
...
echo "=== MazelClaw Auth System Setup ==="
echo ""
...
echo "  2. Subscribe to a topic (e.g., 'mazelclaw-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  MazelClaw message: Send warning via MazelClaw itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/mazelclaw-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/mazelclaw-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/mazelclaw-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now mazelclaw-auth-monitor.timer
...
echo "  - Auth monitor: systemctl --user status mazelclaw-auth-monitor.timer"
echo ""
echo "Quick commands:"
