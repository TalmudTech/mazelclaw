...
`mazelclaw.invoke --each`) without adding new MazelClaw code per workflow.
...
This extension depends on MazelClaw internal modules (the embedded agent runner).
It is intended to ship as a **bundled** MazelClaw extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.mazelclaw/extensions` as a standalone plugin directory.
