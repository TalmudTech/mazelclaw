...
export async function handleIrcInbound(params: {
  ...
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as MazelClawConfig);
  ...
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as MazelClawConfig);
  ...
  const envelopeOptions = core.channel.reply.resolveEnvelopeFormatOptions(config as MazelClawConfig);
