...
import { approveDevicePairing, listDevicePairing } from "mazelclaw/plugin-sdk";
...
function resolveGatewayPort(cfg: MazelClawPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.MAZELCLAW_GATEWAY_PORT?.trim()) ??
...
function resolveAuth(cfg: MazelClawPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: MazelClawPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the MazelClaw iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the MazelClaw iOS app:",
            "",
            "```",
            qrAscii,
            "```",
