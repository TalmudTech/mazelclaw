...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["mazelclawPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["mazelclaw", "pollId"]) ??
    readNestedString(value, ["mazelclaw", "poll", "id"]) ??
    readNestedString(value, ["data", "mazelclawPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "mazelclaw", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        title: "Vote",
        data: {
          mazelclawPollId: pollId,
          pollId,
        },
        msteams: {
          type: "messageBack",
          text: "mazelclaw poll vote",
          displayText: "Vote recorded",
          value: { mazelclawPollId: pollId, pollId },
        },
      },
    ],
  };
