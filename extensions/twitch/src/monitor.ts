...
import type { ReplyPayload, MazelClawConfig } from "mazelclaw/plugin-sdk";
import { createReplyPrefixOptions } from "mazelclaw/plugin-sdk";
...
export type TwitchMonitorOptions = {
  ...
  accountId: string;
  config: unknown; // MazelClawConfig
...
async function processTwitchMessage(params: {
  ...
  const cfg = config as MazelClawConfig;
