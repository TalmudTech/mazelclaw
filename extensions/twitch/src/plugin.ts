/**
 * Twitch channel plugin for MazelClaw.
 *
...
import { buildChannelConfigSchema } from "mazelclaw/plugin-sdk";
 ...
 * for MazelClaw. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: MazelClawConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: MazelClawConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: MazelClawConfig): boolean => {
