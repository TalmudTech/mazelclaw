...
function resolveMazelClawRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.MAZELCLAW_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set MAZELCLAW_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveMazelClawRoot(), "dist", "extensionAPI.js");
