...
MazelClaw extension for Zalo Personal Account messaging via [zca-cli](https://zca-cli.dev).
...
```bash
mazelclaw channels login --channel zalouser
...
```bash
mazelclaw message send --channel zalouser --target <threadId> --message "Hello from MazelClaw!"
```
...
```bash
mazelclaw channels login --channel zalouser              # Login via QR
mazelclaw channels login --channel zalouser --account work
mazelclaw channels status --probe
mazelclaw channels logout --channel zalouser
```

### Directory (IDs, contacts, groups)

```bash
mazelclaw directory self --channel zalouser
mazelclaw directory peers list --channel zalouser --query "name"
mazelclaw directory groups list --channel zalouser --query "work"
mazelclaw directory groups members --channel zalouser --group-id <id>
```
...
```bash
# Text
mazelclaw message send --channel zalouser --target <threadId> --message "message"

# Media (URL)
mazelclaw message send --channel zalouser --target <threadId> --message "caption" --media-url "https://example.com/img.jpg"
```

### Listener

The listener runs inside the Gateway when the channel is enabled. For debugging,
use `mazelclaw channels logs --channel zalouser` or run `zca listen` directly.
...
```bash
mazelclaw channels login --channel zalouser --account work
mazelclaw message send --channel zalouser --account work --target <id> --message "Hello"
