...
metadata: { "mazelclaw": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == MazelClaw `sessions_spawn`
- **File I/O** == MazelClaw `read`/`write`
- **Remote fetch** == MazelClaw `web_fetch` (or `exec` with curl when POST is required)
