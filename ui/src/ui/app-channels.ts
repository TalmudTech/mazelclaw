...
export async function handleWhatsAppStart(host: MazelClawApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: MazelClawApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: MazelClawApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: MazelClawApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: MazelClawApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: MazelClawApp): string {
...
function resolveGatewayHttpAuthHeader(host: MazelClawApp): string | null {
...
function buildGatewayHttpHeaders(host: MazelClawApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: MazelClawApp) {
...
export function handleNostrProfileToggleAdvanced(host: MazelClawApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: MazelClawApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: MazelClawApp) {
  const state = host.nostrProfileFormState;
