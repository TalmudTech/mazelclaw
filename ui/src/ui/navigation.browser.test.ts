...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/mazelclaw/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/mazelclaw");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/mazelclaw/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__MAZELCLAW_CONTROL_UI_BASE_PATH__ = "/mazelclaw";
    const app = mountApp("/mazelclaw/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/mazelclaw");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/mazelclaw/sessions");
  });
