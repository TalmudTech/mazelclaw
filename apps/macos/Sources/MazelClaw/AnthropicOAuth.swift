...
enum AnthropicAuthMode: Equatable {
    ...
    var shortLabel: String {
        switch self {
        case .oauthFile: "OAuth (MazelClaw token file)"
...
enum AnthropicAuthResolver {
    static func resolve(
        ...
        oauthStatus: MazelClawOAuthStore.AnthropicOAuthStatus = MazelClawOAuthStore
...
enum AnthropicOAuth {
    private static let logger = Logger(subsystem: "ai.mazelclaw", category: "anthropic-oauth")
...
enum MazelClawOAuthStore {
    static let oauthFilename = "oauth.json"
    private static let providerKey = "anthropic"
    private static let mazelclawOAuthDirEnv = "MAZELCLAW_OAUTH_DIR"
    ...
    enum AnthropicOAuthStatus: Equatable {
        ...
        var shortDescription: String {
            switch self {
            case .missingFile: "MazelClaw OAuth token file not found"
            case .unreadableFile: "MazelClaw OAuth token file not readable"
            case .invalidJSON: "MazelClaw OAuth token file invalid"
            case .missingProviderEntry: "No Anthropic entry in MazelClaw OAuth token file"
            case .missingTokens: "Anthropic entry missing tokens"
            case .connected: "MazelClaw OAuth credentials found"
            }
        }
    }

    static func oauthDir() -> URL {
        if let override = ProcessInfo.processInfo.environment[self.mazelclawOAuthDirEnv]?
