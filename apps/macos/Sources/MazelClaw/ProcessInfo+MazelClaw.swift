...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.mazelclaw.mac`) even if the shipped
    ...
    {
        if environment["MAZELCLAW_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "mazelclaw.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "mazelclaw.nixMode") { return true }
