...
struct AnthropicAuthControls: View {
    ...
    @State private var oauthStatus: MazelClawOAuthStore.AnthropicOAuthStatus = MazelClawOAuthStore.anthropicOAuthStatus()
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            HStack(spacing: 10) {
                ...
                .disabled(!FileManager().fileExists(atPath: MazelClawOAuthStore.oauthURL().path))
    ...
    private func refresh() {
        let imported = MazelClawOAuthStore.importLegacyAnthropicOAuthIfNeeded()
        self.oauthStatus = MazelClawOAuthStore.anthropicOAuthStatus()
        if imported != nil {
    ...
    private func finishOAuth() async {
        ...
        do {
            ...
            try MazelClawOAuthStore.saveAnthropicOAuth(creds)
            self.refresh()
            self.pkce = nil
            self.code = ""
            self.statusText = "Connected. MazelClaw can now use Claude via OAuth."
        } catch {
...
extension AnthropicAuthControls {
    init(
        ...
        oauthStatus: MazelClawOAuthStore.AnthropicOAuthStatus,
