...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: MazelClawProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.mazelclaw", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: MazelClawKit.AnyCodable]()) {
                $0[$1.key] = MazelClawKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: MazelClawProtocol.AnyCodable?) -> [String: MazelClawProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: MazelClawProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: MazelClawKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: MazelClawProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("mazelclaw.control.heartbeat")
    static let controlAgentEvent = Notification.Name("mazelclaw.control.agent")
}
