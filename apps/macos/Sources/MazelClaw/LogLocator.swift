...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["MAZELCLAW_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/mazelclaw")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("mazelclaw-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("mazelclaw-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/mazelclaw/ (rolling or stdout), or nil if none exist.
