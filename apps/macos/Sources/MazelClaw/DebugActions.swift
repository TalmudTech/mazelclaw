...
enum DebugActions {
    private static let verboseDefaultsKey = "mazelclaw.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "mazelclaw.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = MazelClawPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "MazelClaw", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = MazelClawPaths.configURL
        guard
