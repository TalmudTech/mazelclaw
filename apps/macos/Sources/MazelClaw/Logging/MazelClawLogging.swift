...
enum MazelClawLogging {
    private static let labelSeparator = "::"

    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            let (subsystem, category) = Self.parseLabel(label)
            let osHandler = MazelClawOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = MazelClawFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        MazelClawLogging.bootstrapIfNeeded()
        let label = MazelClawLogging.makeLabel(subsystem: subsystem, category: category)
...
struct MazelClawOSLogHandler: LogHandler {
...
struct MazelClawFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = MazelClawLogging.parseLabel(self.label)
