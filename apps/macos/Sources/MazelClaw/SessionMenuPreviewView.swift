...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.mazelclaw", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> MazelClawSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: MazelClawSessionPreviewEntry,
    ...
    private static func cache(payload: MazelClawSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: MazelClawChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [MazelClawKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [MazelClawKit.AnyCodable]) -> [MazelClawChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(MazelClawChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: MazelClawChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: MazelClawChatMessage) -> Bool {
    ...
    private static func toolNames(for message: MazelClawChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: MazelClawChatMessage) -> String? {
