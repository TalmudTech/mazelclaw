...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.mazelclaw", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: MazelClawPeekabooBridgeServices?
    private static var mazelclawSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("MazelClaw", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = MazelClawPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.mazelclawSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.mazelclawSocketPath, privacy: .public)")
    }
...
@MainActor
private final class MazelClawPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.mazelclaw.peekaboo")
