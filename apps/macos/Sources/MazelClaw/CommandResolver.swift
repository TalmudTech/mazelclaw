...
enum CommandResolver {
    private static let projectRootDefaultsKey = "mazelclaw.gatewayProjectRootPath"
    private static let helperName = "mazelclaw"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let mazelclawEntry = root.appendingPathComponent("mazelclaw.mjs").path
        if FileManager().isReadableFile(atPath: mazelclawEntry) { return mazelclawEntry }
        let binEntry = root.appendingPathComponent("bin/mazelclaw.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/mazelclaw")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let mazelclawPaths = self.mazelclawManagedPaths(home: home)
        if !mazelclawPaths.isEmpty {
            extras.insert(contentsOf: mazelclawPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + mazelclawPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func mazelclawManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".mazelclaw"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func mazelclawExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectMazelClawExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("mazelclaw.mjs").path,
            root.appendingPathComponent("bin/mazelclaw.js").path,
        ]
    ...
    static func hasAnyMazelClawInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.mazelclawExecutable(searchPaths: searchPaths) != nil { return true }
    ...
    static func mazelclawNodeCommand(
        subcommand: String,
        extraArgs: [String] = [],
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let mazelclawPath = self.projectMazelClawExecutable(projectRoot: root) {
                return [mazelclawPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "mazelclaw", subcommand] + extraArgs
            }
            if let mazelclawPath = self.mazelclawExecutable(searchPaths: searchPaths) {
                return [mazelclawPath, subcommand] + extraArgs
            }

            let missingEntry = """
            mazelclaw entrypoint missing (looked for dist/index.js or mazelclaw.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real mazelclaw CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/mazelclaw"
        ...
        \(cliSection)
        \(projectSection)
        if command -v mazelclaw >/dev/null 2>&1; then
          CLI="$(command -v mazelclaw)"
          mazelclaw \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/mazelclaw.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/mazelclaw.mjs"
            node "$PRJ/mazelclaw.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/mazelclaw.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/mazelclaw.js"
            node "$PRJ/bin/mazelclaw.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent mazelclaw"
          pnpm --silent mazelclaw \(quotedArgs);
        else
          echo "mazelclaw CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? MazelClawConfigFile.loadDict()
