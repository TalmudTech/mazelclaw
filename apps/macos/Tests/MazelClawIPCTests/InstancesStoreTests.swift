...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: MazelClawProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: MazelClawProtocol.AnyCodable] = [
            "presence": .init([MazelClawProtocol.AnyCodable(entry)]),
        ]
        let payload = MazelClawProtocol.AnyCodable(payloadMap)
