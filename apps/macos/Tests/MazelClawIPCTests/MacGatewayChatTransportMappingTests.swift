...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: MazelClawProtocol.AnyCodable(["ok": MazelClawProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: MazelClawProtocol.AnyCodable(["ok": MazelClawProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = MazelClawProtocol.AnyCodable([
            "runId": MazelClawProtocol.AnyCodable("run-1"),
            "sessionKey": MazelClawProtocol.AnyCodable("main"),
            "state": MazelClawProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: MazelClawProtocol.AnyCodable(["a": MazelClawProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
