...
import Foundation
import Testing
@testable import MazelClawChatUI
...
private final class TestChatTransport: @unchecked Sendable, MazelClawChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [MazelClawChatHistoryPayload]
    private let sessionsResponses: [MazelClawChatSessionsListResponse]

    private let stream: AsyncStream<MazelClawChatTransportEvent>
    private let continuation: AsyncStream<MazelClawChatTransportEvent>.Continuation

    init(
        historyResponses: [MazelClawChatHistoryPayload],
        sessionsResponses: [MazelClawChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<MazelClawChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> MazelClawChatHistoryPayload {
        ...
        return self.historyResponses.last ?? MazelClawChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [MazelClawChatAttachmentPayload]) async throws -> MazelClawChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return MazelClawChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> MazelClawChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? MazelClawChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: MazelClawChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = MazelClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = MazelClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { MazelClawChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = MazelClawChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = MazelClawChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = MazelClawChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { MazelClawChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
