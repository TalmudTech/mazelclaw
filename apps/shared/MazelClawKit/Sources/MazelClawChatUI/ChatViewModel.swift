...
private let chatUILogger = Logger(subsystem: "ai.mazelclaw", category: "MazelClawChatUI")

@MainActor
@Observable
public final class MazelClawChatViewModel {
    public private(set) var messages: [MazelClawChatMessage] = []
    ...
    public var attachments: [MazelClawPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [MazelClawChatPendingToolCall] = []
    public private(set) var sessions: [MazelClawChatSessionEntry] = []
    private let transport: any MazelClawChatTransport
    ...
    private var pendingToolCallsById: [String: MazelClawChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any MazelClawChatTransport) {
    ...
    public var sessionChoices: [MazelClawChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [MazelClawChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: MazelClawPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [MazelClawChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: MazelClawChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: MazelClawChatMessage) -> MazelClawChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> MazelClawChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return MazelClawChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageIdentityKey(for message: MazelClawChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [MazelClawChatMessage],
        incoming: [MazelClawChatMessage]) -> [MazelClawChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [MazelClawChatMessage]) -> [MazelClawChatMessage] {
        var result: [MazelClawChatMessage] = []
    ...
    private static func dedupeKey(for message: MazelClawChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [MazelClawChatMessageContent] = [
            MazelClawChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> MazelClawChatAttachmentPayload in
            MazelClawChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> MazelClawChatSessionEntry {
        MazelClawChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: MazelClawChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: MazelClawChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: MazelClawAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = MazelClawChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> MazelClawPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
