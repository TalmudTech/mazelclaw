...
final class MotionService: MotionServicing {
    func activities(params: MazelClawMotionActivityParams) async throws -> MazelClawMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[MazelClawMotionActivityEntry], Error>) in
        ...
        return MazelClawMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: MazelClawPedometerParams) async throws -> MazelClawPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<MazelClawPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = MazelClawPedometerPayload(
