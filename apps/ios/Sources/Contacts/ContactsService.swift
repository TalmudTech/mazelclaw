...
final class ContactsService: ContactsServicing {
    ...
    func search(params: MazelClawContactsSearchParams) async throws -> MazelClawContactsSearchPayload {
        ...
        return MazelClawContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: MazelClawContactsAddParams) async throws -> MazelClawContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return MazelClawContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return MazelClawContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> MazelClawContactPayload {
        MazelClawContactPayload(
