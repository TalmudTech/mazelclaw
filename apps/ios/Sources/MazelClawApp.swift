...
@MainActor
final class MazelClawAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.mazelclaw.ios", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.mazelclaw.ios", category: "BackgroundWake")
    private static let wakeRefreshTaskIdentifier = "ai.mazelclaw.ios.bgrefresh"
...
enum WatchPromptNotificationBridge {
    static let typeKey = "mazelclaw.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "mazelclaw.watch.promptId"
    static let sessionKeyKey = "mazelclaw.watch.sessionKey"
    static let actionPrimaryIDKey = "mazelclaw.watch.action.primary.id"
    static let actionPrimaryLabelKey = "mazelclaw.watch.action.primary.label"
    static let actionSecondaryIDKey = "mazelclaw.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "mazelclaw.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "mazelclaw.watch.action.primary"
    static let actionSecondaryIdentifier = "mazelclaw.watch.action.secondary"
    static let categoryPrefix = "mazelclaw.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: MazelClawWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> MazelClawWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return MazelClawWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "MazelClaw" : title
        content.body = body
    ...
    private static func categoryActions(
        primaryAction: MazelClawWatchAction,
        secondaryAction: MazelClawWatchAction?) -> [UNNotificationAction]
    {
...
@main
struct MazelClawApp: App {
    @State private var appModel: NodeAppModel
    @State private var gatewayController: GatewayConnectionController
    @UIApplicationDelegateAdaptor(MazelClawAppDelegate.self) private var appDelegate
...
extension MazelClawApp {
    private static func installUncaughtExceptionLogger() {
        NSLog("MazelClaw: installing uncaught exception handler")
