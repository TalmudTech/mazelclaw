...
final class RemindersService: RemindersServicing {
    func list(params: MazelClawRemindersListParams) async throws -> MazelClawRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[MazelClawReminderPayload], Error>) in
            store.fetchReminders(matching: predicate) { items in
                ...
                let payload = selected.map { reminder in
                    ...
                    return MazelClawReminderPayload(
        ...
        return MazelClawRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: MazelClawRemindersAddParams) async throws -> MazelClawRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = MazelClawReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return MazelClawRemindersAddPayload(reminder: payload)
    }
