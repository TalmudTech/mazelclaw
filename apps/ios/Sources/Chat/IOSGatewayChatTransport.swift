...
struct IOSGatewayChatTransport: MazelClawChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.mazelclaw", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> MazelClawChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(MazelClawChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> MazelClawChatHistoryPayload {
        ...
        return try JSONDecoder().decode(MazelClawChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [MazelClawChatAttachmentPayload]) async throws -> MazelClawChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [MazelClawChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(MazelClawChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(MazelClawGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: MazelClawGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: MazelClawChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: MazelClawAgentEventPayload.self)
                        {
