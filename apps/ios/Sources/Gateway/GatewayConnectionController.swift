...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [MazelClawCapability.canvas.rawValue, MazelClawCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(MazelClawCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(MazelClawCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = MazelClawLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(MazelClawCapability.location.rawValue) }

        caps.append(MazelClawCapability.device.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(MazelClawCapability.watch.rawValue)
        }
        caps.append(MazelClawCapability.photos.rawValue)
        caps.append(MazelClawCapability.contacts.rawValue)
        caps.append(MazelClawCapability.calendar.rawValue)
        caps.append(MazelClawCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(MazelClawCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            MazelClawScreenCommand.record.rawValue,
            MazelClawSystemCommand.notify.rawValue,
            MazelClawChatCommand.push.rawValue,
        ...
        if caps.contains(MazelClawCapability.camera.rawValue) {
            commands.append(MazelClawCameraCommand.list.rawValue)
            commands.append(MazelClawCameraCommand.snap.rawValue)
            commands.append(MazelClawCameraCommand.clip.rawValue)
        }
        if caps.contains(MazelClawCapability.location.rawValue) {
            commands.append(MazelClawLocationCommand.get.rawValue)
        }
        if caps.contains(MazelClawCapability.device.rawValue) {
            commands.append(MazelClawDeviceCommand.status.rawValue)
            commands.append(MazelClawDeviceCommand.info.rawValue)
        }
        if caps.contains(MazelClawCapability.watch.rawValue) {
            commands.append(MazelClawWatchCommand.status.rawValue)
            commands.append(MazelClawWatchCommand.notify.rawValue)
        }
        if caps.contains(MazelClawCapability.photos.rawValue) {
            commands.append(MazelClawPhotosCommand.latest.rawValue)
        }
        if caps.contains(MazelClawCapability.contacts.rawValue) {
            commands.append(MazelClawContactsCommand.search.rawValue)
            commands.append(MazelClawContactsCommand.add.rawValue)
        }
        if caps.contains(MazelClawCapability.calendar.rawValue) {
            commands.append(MazelClawCalendarCommand.events.rawValue)
            commands.append(MazelClawCalendarCommand.add.rawValue)
        }
        if caps.contains(MazelClawCapability.reminders.rawValue) {
            commands.append(MazelClawRemindersCommand.list.rawValue)
            commands.append(MazelClawRemindersCommand.add.rawValue)
        }
        if caps.contains(MazelClawCapability.motion.rawValue) {
            commands.append(MazelClawMotionCommand.activity.rawValue)
            commands.append(MazelClawMotionCommand.pedometer.rawValue)
        }
