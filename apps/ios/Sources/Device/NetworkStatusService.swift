...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> MazelClawNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> MazelClawNetworkStatusPayload {
        let status: MazelClawNetworkPathStatus = switch path.status {
        ...
        var interfaces: [MazelClawNetworkInterfaceType] = []
        if path.usesInterfaceType(.wifi) { interfaces.append(.wifi) }
        if path.usesInterfaceType(.cellular) { interfaces.append(.cellular) }
        if path.usesInterfaceType(.wiredEthernet) { interfaces.append(.wired) }
        if interfaces.isEmpty { interfaces.append(.other) }

        return MazelClawNetworkStatusPayload(
            status: status,
    ...
    private static func fallbackPayload() -> MazelClawNetworkStatusPayload {
        MazelClawNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
