...
import type { MazelClawConfig } from "../src/config/config.js";
import { resolvePreferredMazelClawTmpDir } from "../src/infra/tmp-mazelclaw-dir.js";
...
const makeTempDir = async (prefix: string) => {
  const baseDir = resolvePreferredMazelClawTmpDir();
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("mazelclaw-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    await withEnvSnapshot(async () => {
      const binDir = await createTrackedTempDir(tempPaths, "mazelclaw-bin-sherpa-");
      const modelDir = await createTrackedTempDir(tempPaths, "mazelclaw-sherpa-model-");
      ...
      const cfg: MazelClawConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    await withEnvSnapshot(async () => {
      const binDir = await createTrackedTempDir(tempPaths, "mazelclaw-bin-whispercpp-");
      const modelDir = await createTrackedTempDir(tempPaths, "mazelclaw-whispercpp-model-");
  ...
  it("uses gemini CLI for images when available", async () => {
    await withEnvSnapshot(async () => {
      const binDir = await createTrackedTempDir(tempPaths, "mazelclaw-bin-gemini-");
      ...
      const cfg: MazelClawConfig = { tools: { media: { image: {} } } };
  ...
  it("skips auto-detect when no supported binaries are available", async () => {
    await withEnvSnapshot(async () => {
      const emptyBinDir = await createTrackedTempDir(tempPaths, "mazelclaw-bin-empty-");
