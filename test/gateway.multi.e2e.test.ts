...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `mazelclaw-e2e-${name}-`));
  const configDir = path.join(homeDir, ".mazelclaw");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          MAZELCLAW_SKIP_BROWSER_CONTROL_SERVER: "1",
    ...
    async () => {
      ...
      const [hookResA, hookResB] = await Promise.all([
        postJson(
          ...
          { "x-mazelclaw-token": gwA.hookToken },
        ),
        postJson(
          ...
          { "x-mazelclaw-token": gwB.hookToken },
        ),
      ]);
