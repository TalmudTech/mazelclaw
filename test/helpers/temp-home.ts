...
type EnvSnapshot = {
  ...
  mazelclawHome: string | undefined;
...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    mazelclawHome: process.env.MAZELCLAW_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("MAZELCLAW_HOME", snapshot.mazelclawHome);
  restoreKey("MAZELCLAW_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked MAZELCLAW_HOME.
  delete process.env.MAZELCLAW_HOME;
  process.env.MAZELCLAW_STATE_DIR = path.join(base, ".mazelclaw");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "mazelclaw-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".mazelclaw", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
