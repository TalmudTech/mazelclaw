...
- Publish as `mazelclaw/plugin-sdk` (or export from core under `mazelclaw/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `MazelClawPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: MazelClawConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: MazelClawConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: MazelClawConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: MazelClawConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: MazelClawConfig): string;
  };
};
```
...
- Introduce `mazelclaw/plugin-sdk`.
- Add `api.runtime` to `MazelClawPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `mazelclawRuntime: ">=2026.2.0"`).
