...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.mazelclaw/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.mazelclaw/node.json` (node id + pairing token).
...
Path: `~/.mazelclaw/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.mazelclaw/exec-approvals.sock",
...
- Unix socket at `~/.mazelclaw/exec-approvals.sock` (0600).
