...
read_when:
  - Designing workspace memory (~/.mazelclaw/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep MazelClaw integration
...
Target: Clawd-style workspace (`agents.defaults.workspace`, default `~/.mazelclaw/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.mazelclaw/workspace` as canonical human-readable memory.
...
```
~/.mazelclaw/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in MazelClaw**, but keep a separable core library.

### Why integrate into MazelClaw?

- MazelClaw already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `mazelclaw memory recall "…" --k 25 --since 30d`
  - `mazelclaw memory reflect --since 7d`
...
Pragmatic take for `~/.mazelclaw/workspace`:
