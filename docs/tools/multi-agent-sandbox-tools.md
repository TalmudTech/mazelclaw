...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `mazelclaw sandbox explain`.
...
Legacy `agent.*` configs are migrated by `mazelclaw doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=mazelclaw-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${MAZELCLAW_STATE_DIR:-$HOME/.mazelclaw}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
