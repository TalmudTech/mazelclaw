---
summary: "Chrome extension: let MazelClaw drive your existing Chrome tab"
read_when:
...
The MazelClaw Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate mazelclaw-managed Chrome profile.
...
MazelClaw then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the MazelClaw release (npm package) as static files. There is no separate “build” step.

After upgrading MazelClaw:

- Re-run `mazelclaw browser extension install` to refresh the installed files under your MazelClaw state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (set gateway token once)

MazelClaw ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.

Before first attach, open extension Options and set:

- `Port` (default `18792`)
- `Gateway token` (must match `gateway.auth.token` / `MAZELCLAW_GATEWAY_TOKEN`)
...
- Open the tab you want MazelClaw to control.
...
- `ON`: attached; MazelClaw can drive that tab.
...
Debugging: `mazelclaw sandbox explain`
...
`mazelclaw browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `mazelclaw browser extension install` first to copy the extension to a stable location under your MazelClaw state directory.
...
- **This is not isolated** like the dedicated mazelclaw-managed profile.
