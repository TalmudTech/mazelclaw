...
你可以在 `~/.mazelclaw/mazelclaw.json` 中覆盖此设置：
...
所有日志配置都在 `~/.mazelclaw/mazelclaw.json` 的 `logging` 下。
...
- MazelClaw 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
- 你也可以使用 `mazelclaw plugins enable diagnostics-otel` 启用插件。
...
- `mazelclaw.tokens`（计数器，属性：`mazelclaw.token`、`mazelclaw.channel`、`mazelclaw.provider`、`mazelclaw.model`）
- `mazelclaw.cost.usd`（计数器，属性：`mazelclaw.channel`、`mazelclaw.provider`、`mazelclaw.model`）
- `mazelclaw.run.duration_ms`（直方图，属性：`mazelclaw.channel`、`mazelclaw.provider`、`mazelclaw.model`）
- `mazelclaw.context.tokens`（直方图，属性：`mazelclaw.context`、`mazelclaw.channel`、`mazelclaw.provider`、`mazelclaw.model`）

消息流：

- `mazelclaw.webhook.received`（计数器，属性：`mazelclaw.channel`、`mazelclaw.webhook`）
- `mazelclaw.webhook.error`（计数器，属性：`mazelclaw.channel`、`mazelclaw.webhook`）
- `mazelclaw.webhook.duration_ms`（直方图，属性：`mazelclaw.channel`、`mazelclaw.webhook`）
- `mazelclaw.message.queued`（计数器，属性：`mazelclaw.channel`、`mazelclaw.source`）
- `mazelclaw.message.processed`（计数器，属性：`mazelclaw.channel`、`mazelclaw.outcome`）
- `mazelclaw.message.duration_ms`（直方图，属性：`mazelclaw.channel`、`mazelclaw.outcome`）

队列 + 会话：

- `mazelclaw.queue.lane.enqueue`（计数器，属性：`mazelclaw.lane`）
- `mazelclaw.queue.lane.dequeue`（计数器，属性：`mazelclaw.lane`）
- `mazelclaw.queue.depth`（直方图，属性：`mazelclaw.lane` 或 `mazelclaw.channel=heartbeat`）
- `mazelclaw.queue.wait_ms`（直方图，属性：`mazelclaw.lane`）
- `mazelclaw.session.state`（计数器，属性：`mazelclaw.state`、`mazelclaw.reason`）
- `mazelclaw.session.stuck`（计数器，属性：`mazelclaw.state`）
- `mazelclaw.session.stuck_age_ms`（直方图，属性：`mazelclaw.state`）
- `mazelclaw.run.attempt`（计数器，属性：`mazelclaw.attempt`）
...
- `mazelclaw.webhook.error`
  - `mazelclaw.channel`、`mazelclaw.webhook`、`mazelclaw.chatId`、`mazelclaw.error`
- `mazelclaw.message.processed`
  - `mazelclaw.channel`、`mazelclaw.outcome`、`mazelclaw.chatId`、`mazelclaw.messageId`、`mazelclaw.sessionKey`、`mazelclaw.sessionId`、`mazelclaw.reason`
- `mazelclaw.session.stuck`
  - `mazelclaw.state`、`mazelclaw.ageMs`、`mazelclaw.queueDepth`、`mazelclaw.sessionKey`、`mazelclaw.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `mazelclaw doctor`。
