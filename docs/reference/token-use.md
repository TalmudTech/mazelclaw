---
summary: "How MazelClaw builds prompt context and reports token usage + costs"
read_when:
...
MazelClaw tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

MazelClaw assembles its own system prompt on every run. It includes:
...
For images, MazelClaw downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from MazelClaw session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `mazelclaw status --usage` and `mazelclaw channels list` show
...
`cacheWrite`. If pricing is missing, MazelClaw shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. MazelClaw can
...
Anthropic's 1M context window is currently beta-gated. MazelClaw can inject the
