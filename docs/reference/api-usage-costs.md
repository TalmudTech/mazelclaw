...
MazelClaw features that can generate provider usage or paid API calls.
...
- `mazelclaw status --usage` and `mazelclaw channels list` show provider **usage windows**
...
MazelClaw can pick up credentials from:
...
- `mazelclaw status --usage`
- `mazelclaw models status --json`
...
`mazelclaw models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
