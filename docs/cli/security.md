---
summary: "CLI reference for `mazelclaw security` (audit and fix common security footguns)"
read_when:
...
It also warns when existing sandbox browser Docker containers have missing/stale hash labels (for example pre-migration containers missing `mazelclaw.browserConfigEpoch`) and recommends `mazelclaw sandbox recreate --browser --all`.
...
```bash
mazelclaw security audit --json | jq '.summary'
mazelclaw security audit --deep --json | jq '.findings[] | select(.severity=="critical") | .checkId'
```

If `--fix` and `--json` are combined, output includes both fix actions and final report:

```bash
mazelclaw security audit --fix --json | jq '{fix: .fix.ok, summary: .report.summary}'
```
