...
- During heartbeat runs, MazelClaw treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `mazelclaw sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, MazelClaw skips the heartbeat run entirely (no model call).
...
```bash
mazelclaw system event --text "Check for urgent follow-ups" --mode now
```
