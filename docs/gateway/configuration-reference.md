...
description: "Complete field-by-field reference for ~/.mazelclaw/mazelclaw.json"
---

# Configuration Reference

Every field available in `~/.mazelclaw/mazelclaw.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — MazelClaw uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `mazelclaw doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["mazelclaw-dm"] },
      guilds: {
        "123456789012345678": {
...
Mattermost ships as a plugin: `mazelclaw plugins install @mazelclaw/mattermost`.
...
MazelClaw spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@mazelclaw"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `mazelclaw.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, MazelClaw auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, MazelClaw assumes `anthropic` (deprecated).
    ...
    defaults: {
      sandbox: {
        ...
        browser: {
          ...
          network: "mazelclaw-sandbox-browser",
          cdpPort: 9222,
...
- `none`: per-scope sandbox workspace under `~/.mazelclaw/sandboxes`
...
noVNC observer access uses VNC auth by default and MazelClaw emits a short-lived token URL (instead of exposing the password in the shared URL).

- `allowHostControl: false` (default) blocks sandboxed sessions from targeting the host browser.
- `network` defaults to `mazelclaw-sandbox-browser` (dedicated bridge network). Set to `bridge` only when you explicitly want global bridge connectivity.
    ...
    list: [
      {
        ...
        default: true,
        name: "Main Agent",
        workspace: "~/.mazelclaw/workspace",
        agentDir: "~/.mazelclaw/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@mazelclaw"] },
        sandbox: { mode: "off" },
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:mazelclaw`   | All built-in tools (excludes provider plugins)                                           |
...
MazelClaw uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.mazelclaw/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `MAZELCLAW_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `mazelclaw onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `mazelclaw onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `mazelclaw onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `mazelclaw onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `mazelclaw onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `mazelclaw onboard --auth-choice minimax-api`.
...
- Loaded from `~/.mazelclaw/extensions`, `<workspace>/.mazelclaw/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or MAZELCLAW_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `MAZELCLAW_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.mazelclaw-dev` + port `19001`), `--profile <name>` (uses `~/.mazelclaw-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.mazelclaw/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-mazelclaw-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `MAZELCLAW_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or MAZELCLAW_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__mazelclaw__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__mazelclaw__/a2ui/`
...
- Also serves A2UI at `/__mazelclaw__/a2ui/`.
...
- Hostname defaults to `mazelclaw`. Override with `MAZELCLAW_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.mazelclaw/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `mazelclaw dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.mazelclaw/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${MAZELCLAW_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.mazelclaw/credentials/oauth.json`.
...
- Default log file: `/tmp/mazelclaw/mazelclaw-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `mazelclaw doctor --fix` can strip unknown keys).
