...
`mazelclaw doctor` is the repair + migration tool for MazelClaw. It fixes stale
...
- Extra workspace dir detection (`~/mazelclaw`).
...
you to run `mazelclaw doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.mazelclaw/mazelclaw.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.mazelclaw/sessions/` to `~/.mazelclaw/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.mazelclaw/agent/` to `~/.mazelclaw/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.mazelclaw/credentials/*.json` (except `oauth.json`)
  - to `~/.mazelclaw/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `mazelclaw doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.mazelclaw` folders exist across
  home directories or when `MAZELCLAW_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.mazelclaw/mazelclaw.json` is
...
offers to remove them and install the MazelClaw service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named MazelClaw gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `mazelclaw doctor --generate-gateway-token` to force token
...
- `mazelclaw doctor` prompts before rewriting supervisor config.
- `mazelclaw doctor --yes` accepts the default repair prompts.
- `mazelclaw doctor --repair` applies recommended fixes without prompts.
- `mazelclaw doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `mazelclaw gateway install --force`.
