...
read_when:
  - Running MazelClaw behind an identity-aware proxy
  - Setting up Pomerium, Caddy, or nginx with OAuth in front of MazelClaw
...
- You run MazelClaw behind an **identity-aware proxy** (Pomerium, Caddy + OAuth, nginx + oauth2-proxy, Traefik + forward auth)
...
3. MazelClaw checks that the request came from a **trusted proxy IP** (configured in `gateway.trustedProxies`)
4. MazelClaw extracts the user identity from the configured header
...
```yaml
routes:
  - from: https://mazelclaw.example.com
    to: http://mazelclaw-gateway:18789
    policy:
      - allow:
          or:
            - email:
...
```
mazelclaw.example.com {
    authenticate with oauth2_provider
    authorize with policy1

    reverse_proxy mazelclaw:18789 {
...
location / {
    ...
    proxy_pass http://mazelclaw:18789;
...
`mazelclaw security audit` will flag trusted-proxy auth with a **critical** severity finding. This is intentional — it's a reminder that you're delegating security to your proxy setup.
...
3. Update MazelClaw config with trusted-proxy auth
4. Restart the Gateway
5. Test WebSocket connections from the Control UI
6. Run `mazelclaw security audit` and review findings
