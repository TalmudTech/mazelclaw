...
MazelClaw has two distinct problems that look similar on the surface:

1. **Operator remote control**: the macOS menu bar app controlling a gateway running elsewhere.
2. **Node pairing**: iOS/Android (and future nodes) finding a gateway and pairing securely.

The design goal is to keep all network discovery/advertising in the **Node Gateway** (`mazelclaw gateway`) and keep clients (mac app, iOS) as consumers.
...
- Service types:
  - `_mazelclaw-gw._tcp` (gateway transport beacon)
- TXT keys (non-secret):
  - `role=gateway`
  ...
  - `cliPath=<path>` (optional; absolute path to a runnable `mazelclaw` entrypoint or binary)
...
- `MAZELCLAW_DISABLE_BONJOUR=1` disables advertising.
- `gateway.bind` in `~/.mazelclaw/mazelclaw.json` controls the Gateway bind mode.
- `MAZELCLAW_SSH_PORT` overrides the SSH port advertised in TXT (defaults to 22).
- `MAZELCLAW_TAILNET_DNS` publishes a `tailnetDns` hint (MagicDNS).
- `MAZELCLAW_CLI_PATH` overrides the advertised CLI path.
