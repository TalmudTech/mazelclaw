...
service via the external `mazelclaw` CLI (no embedded runtime). This gives you
...
- The app installs a per‑user LaunchAgent labeled `bot.molt.gateway`
  (or `bot.molt.<profile>` when using `--profile`/`MAZELCLAW_PROFILE`; legacy `com.mazelclaw.*` is supported).
