---
summary: "MazelClaw on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up MazelClaw on a Raspberry Pi
  - Running MazelClaw on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# MazelClaw on Raspberry Pi

## Goal

Run a persistent, always-on MazelClaw Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most MazelClaw features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled mazelclaw

# Enable if not
sudo systemctl enable mazelclaw

# Start on boot
sudo systemctl start mazelclaw
```
...
```bash
# Check logs
journalctl -u mazelclaw --no-pager -n 100

# Common fix: rebuild
cd ~/mazelclaw  # if using hackable install
npm run build
sudo systemctl restart mazelclaw
```
