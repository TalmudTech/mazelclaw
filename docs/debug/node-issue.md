...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in MazelClaw
...
Running MazelClaw via Node with `tsx` fails at startup with:

```
[mazelclaw] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `mazelclaw status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch mazelclaw.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node mazelclaw.mjs status` works on Node 25.
