...
Deploy MazelClaw on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/mazelclaw/mazelclaw)
...
repository configures everything needed to run MazelClaw:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: MAZELCLAW_STATE_DIR
        value: /data/.mazelclaw
      - key: MAZELCLAW_WORKSPACE_DIR
        value: /data/workspace
      - key: MAZELCLAW_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/mazelclaw`.
...
If you use the original MazelClaw repository, Render will not auto-deploy your MazelClaw. To update it, run a manual Blueprint sync from the dashboard.
...
For MazelClaw, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any MazelClaw host.
