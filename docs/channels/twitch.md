...
read_when:
  - Setting up Twitch chat integration for MazelClaw
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. MazelClaw connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
mazelclaw plugins install @mazelclaw/twitch
```

Local checkout (when running from a git repo):

```bash
mazelclaw plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `MAZELCLAW_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "mazelclaw", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use MAZELCLAW_TWITCH_ACCESS_TOKEN env var)
...
```bash
MAZELCLAW_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
