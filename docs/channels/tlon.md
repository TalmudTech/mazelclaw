...
Tlon is a decentralized messenger built on Urbit. MazelClaw connects to your Urbit ship and can
...
```bash
mazelclaw plugins install @mazelclaw/tlon
```

Local checkout (when running from a git repo):

```bash
mazelclaw plugins install ./extensions/tlon
```
...
By default, MazelClaw blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `mazelclaw message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, MazelClaw replies in-thread.
