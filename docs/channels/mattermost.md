---
summary: "Mattermost bot setup and MazelClaw config"
read_when:
...
```bash
mazelclaw plugins install @mazelclaw/mattermost
```

Local checkout (when running from a git repo):

```bash
mazelclaw plugins install ./extensions/mattermost
```
...
4. Configure MazelClaw and start the gateway.
...
- Approve via:
  - `mazelclaw pairing list mattermost`
  - `mazelclaw pairing approve mattermost <CODE>`
...
Use these target formats with `mazelclaw message send` or cron/webhooks:
