...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects MazelClaw to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
mazelclaw plugins install @mazelclaw/feishu
```

Local checkout (when running from a git repo):

```bash
mazelclaw plugins install ./extensions/feishu
```
...
If you just installed MazelClaw, run the wizard:
...
2. Configuring app credentials in MazelClaw
...
1. You already ran `mazelclaw channels add` for Feishu
2. The gateway is running (`mazelclaw gateway status`)
...
```bash
mazelclaw pairing approve feishu <CODE>
```
...
2. Run `mazelclaw logs --follow` and look for `chat_id`
...
2. Run `mazelclaw logs --follow` and look for `open_id`
...
| Command                    | Description                   |
| -------------------------- | ----------------------------- |
| `mazelclaw gateway status`  | Show gateway status           |
| `mazelclaw gateway install` | Install/start gateway service |
| `mazelclaw gateway stop`    | Stop gateway service          |
| `mazelclaw gateway restart` | Restart gateway service       |
| `mazelclaw logs --follow`   | Tail gateway logs             |
...
4. Check logs: `mazelclaw logs --follow`
...
5. Ensure the gateway is running: `mazelclaw gateway status`
6. Check logs: `mazelclaw logs --follow`
  ...
  agents: {
    list: [
      { id: "main" },
      {
        ...
        agentDir: "/home/user/.mazelclaw/agents/clawd-fan/agent",
      },
      {
        id: "clawd-xi",
        workspace: "/home/user/clawd-xi",
        agentDir: "/home/user/.mazelclaw/agents/clawd-xi/agent",
      },
    ],
  },
  bindings: [
