...
Matrix is an open, decentralized messaging protocol. MazelClaw connects as a Matrix **user**
...
```bash
mazelclaw plugins install @mazelclaw/matrix
```

Local checkout (when running from a git repo):

```bash
mazelclaw plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `mazelclaw plugins install @mazelclaw/matrix`
   - From a local checkout: `mazelclaw plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: MazelClaw calls the same
     login endpoint, stores the access token in `~/.mazelclaw/credentials/matrix/credentials.json`,
...
- On first connection, MazelClaw requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  MazelClaw logs a warning.
...
`~/.mazelclaw/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `mazelclaw pairing list matrix`
  - `mazelclaw pairing approve matrix <CODE>`
...
- On startup, MazelClaw resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
