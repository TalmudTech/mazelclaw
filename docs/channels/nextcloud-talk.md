...
```bash
mazelclaw plugins install @mazelclaw/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
mazelclaw plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "MazelClaw" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `mazelclaw pairing list nextcloud-talk`
  - `mazelclaw pairing approve nextcloud-talk <CODE>`
