...
- Jobs persist under `~/.mazelclaw/cron/` so restarts don’t lose schedules.
...
mazelclaw cron list
mazelclaw cron run <job-id>
mazelclaw cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.mazelclaw/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `mazelclaw cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, MazelClaw applies a
...
If `delivery` is omitted for isolated jobs, MazelClaw defaults to `announce`.
...
- Job store: `~/.mazelclaw/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.mazelclaw/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
```json5
{
  cron: {
    ...
    store: "~/.mazelclaw/cron/jobs.json",
...
mazelclaw cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
mazelclaw cron edit <jobId> --agent ops
mazelclaw cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
mazelclaw cron run <jobId>
mazelclaw cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
mazelclaw cron edit <jobId> \
...
```bash
mazelclaw cron edit <jobId> --exact
```
...
```bash
mazelclaw system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`mazelclaw system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `MAZELCLAW_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- MazelClaw applies exponential retry backoff for recurring jobs after consecutive errors:
