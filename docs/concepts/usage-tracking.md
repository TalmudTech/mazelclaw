...
- `/usage cost` in chats: local cost summary aggregated from MazelClaw session logs.
- CLI: `mazelclaw status --usage` prints a full per-provider breakdown.
- CLI: `mazelclaw channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
