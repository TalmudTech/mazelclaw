...
```bash
mazelclaw tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `mazelclaw tui --deliver`
...
- Check the Gateway logs: `mazelclaw logs --follow`.
- Confirm the agent can run: `mazelclaw status` and `mazelclaw models status`.
...
- No agents in picker: check `mazelclaw agents list` and your routing config.
