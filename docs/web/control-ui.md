...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/mazelclaw`)
...
If the page fails to load, start the Gateway first: `mazelclaw gateway`.
...
you revoke it with `mazelclaw devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.mazelclaw/mazelclaw.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. MazelClaw
...
MazelClaw **blocks** Control UI connections without device identity.
