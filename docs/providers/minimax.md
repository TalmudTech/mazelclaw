---
summary: "Use MiniMax M2.1 in MazelClaw"
read_when:
  - You want MiniMax models in MazelClaw
...
```bash
mazelclaw plugins enable minimax-portal-auth  # skip if already loaded.
mazelclaw gateway restart  # restart if gateway is already running
mazelclaw onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/mazelclaw/mazelclaw/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `mazelclaw.json`:
...
- Use `mazelclaw models list` and `mazelclaw models set minimax/MiniMax-M2.1` to switch.
...
- Running `mazelclaw configure` and selecting **MiniMax M2.1**, or
