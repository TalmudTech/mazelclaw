---
summary: "Use Venice AI privacy-focused models in MazelClaw"
read_when:
  - You want privacy-focused inference in MazelClaw
...
```bash
mazelclaw onboard --auth-choice venice-api-key
```
...
```bash
mazelclaw chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, MazelClaw shows all available Venice models. Pick based on your needs:
...
```bash
mazelclaw models set venice/claude-opus-45
mazelclaw models set venice/llama-3.3-70b
```

List all available models:

```bash
mazelclaw models list | grep venice
```

## Configure via `mazelclaw configure`

1. Run `mazelclaw configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
MazelClaw automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
mazelclaw chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
mazelclaw chat --model venice/claude-opus-45

# Use uncensored model
mazelclaw chat --model venice/venice-uncensored

# Use vision model with image
mazelclaw chat --model venice/qwen3-vl-235b-a22b

# Use coding model
mazelclaw chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `mazelclaw models list` to see currently available models. Some models may be temporarily offline.
